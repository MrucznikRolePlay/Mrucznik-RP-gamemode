	...
	if (IsAPolicja(playerid) || IsAHA(playerid))
	{
		...
		if(IsPlayerConnected(giveplayerid))
		{
			if(GetDistanceBetweenPlayers(giveplayerid,playerid) < 10)
			{
				...
				{
					format(string, sizeof(string), "Gracz %s jest pod wp≥ywem narkotykÛw (ma≥o).", giveplayer);
					SendClientMessage(playerid, COLOR_LIGHTRED, string);
				}
				else if(PlayerStoned[giveplayerid] >= 3)
				{
					format(string, sizeof(string), "Gracz %s jest pod wp≥ywem narkotykÛw (duøo).", giveplayer);
					SendClientMessage(playerid, COLOR_LIGHTRED, string);
				}
				else
				{
