...
YCMD:sms(playerid, params[], help)
{
	...
	if(givePlayerNumber != 555) 
	{
		...
		SavePlayerSentMessage(playerid, sprintf("%s wys³a³ SMS do %s: %s", GetNick(playerid), GetNick(checkNumberPlayer), messSMS));
		SavePlayerSentMessage(checkNumberPlayer, sprintf("%s otrzyma³ SMS od %s: %s", GetNick(checkNumberPlayer), GetNick(playerid), messSMS));
