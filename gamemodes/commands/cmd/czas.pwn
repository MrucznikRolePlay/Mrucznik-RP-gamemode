...
YCMD:czas(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
	{
		...
		if (minuite < 10)
		{
			...
			{
				format(string, sizeof(string), "~y~%d %s %d ~n~~g~|~w~%d:0%d~g~|~n~~w~Czas Aresztu: %d sek", day, mtext, year, hour, minuite, PlayerInfo[playerid][pJailTime]);
			}
			else
			{
		...
		else
		{
			if (PlayerInfo[playerid][pJailTime] > 0)
			{
				format(string, sizeof(string), "~y~%d %s %d~n~~g~|~w~%d:%d~g~|~n~~w~Czas Aresztu: %d sec", day, mtext, year, hour, minuite, PlayerInfo[playerid][pJailTime]);
			}
			else
			{
