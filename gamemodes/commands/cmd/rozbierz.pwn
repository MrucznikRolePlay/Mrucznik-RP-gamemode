//-----------------------------------------------<< Komenda >>-----------------------------------------------//
//-------------------------------------------------[ naucz ]-------------------------------------------------//
//----------------------------------------------------*------------------------------------------------------//
//----[                                                                                                 ]----//
//----[         |||||             |||||                       ||||||||||       ||||||||||               ]----//
//----[        ||| |||           ||| |||                      |||     ||||     |||     ||||             ]----//
//----[       |||   |||         |||   |||                     |||       |||    |||       |||            ]----//
//----[       ||     ||         ||     ||                     |||       |||    |||       |||            ]----//
//----[      |||     |||       |||     |||                    |||     ||||     |||     ||||             ]----//
//----[      ||       ||       ||       ||     __________     ||||||||||       ||||||||||               ]----//
//----[     |||       |||     |||       |||                   |||    |||       |||                      ]----//
//----[     ||         ||     ||         ||                   |||     ||       |||                      ]----//
//----[    |||         |||   |||         |||                  |||     |||      |||                      ]----//
//----[    ||           ||   ||           ||                  |||      ||      |||                      ]----//
//----[   |||           ||| |||           |||                 |||      |||     |||                      ]----//
//----[  |||             |||||             |||                |||       |||    |||                      ]----//
//----[                                                                                                 ]----//
//----------------------------------------------------*------------------------------------------------------//

// Opis:
/*
	
*/


// Notatki skryptera:
/*
	
*/

YCMD:rozbierz(playerid, params[], help)
{
    new colorID; 
    if(isNaked[playerid])
    {
        UndressPlayer(playerid, true);
        return 1;
    }
    
    if( sscanf(params, "k<fix>", colorID))
    {
        sendTipMessage(playerid, "U¿yj /rozbierz [1 - bia³y || 2 - czarny]");
        return 1;
    }

    if(GetPlayerVirtualWorld(playerid) == 40 || PlayerInfo[playerid][pDomWKJ] != 0 || (GetPlayerVirtualWorld(playerid) == 20 && PlayerInfo[playerid][pLocal] == 20)) //ostatni warunek to INT SEKTA
    {
        UndressPlayer(playerid,false, colorID); 
    }
    else
    {
        sendTipMessage(playerid, "Rozebraæ siê mo¿esz tylko w klubie ze striptizem i domu!"); 
    }
    return 1;
}
