	...
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			if(IsPlayerConnected(giveplayerid))
			{
				...
				{
					if(PlayerInfo[playerid][BankMoney] < value)
					{	
					...
					if(PlayerInfo[giveplayerid][BankMoney]+value > MAX_MONEY_IN_BANK)
					{
						sendErrorMessage(playerid, "Gracz do którego próbowa³eœ przelaæ gotówkê - ma zbyt du¿o pieniêdzy na koncie."); 
						return 1;
					}
					//Czynnoœci:
					PlayerInfo[playerid][BankMoney] -= value;
					PlayerInfo[giveplayerid][BankMoney] += value;
