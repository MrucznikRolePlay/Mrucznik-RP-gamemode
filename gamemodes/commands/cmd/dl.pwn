...
YCMD:dl(playerid, params[], help)
{
    ...
    {
        if(IsAnInstructor(playerid) || PlayerInfo[playerid][Admin] >= 5000 || IsAScripter(playerid))
        {
            if(PlayerInfo[playerid][pLocal] == 108 || PlayerInfo[playerid][Admin] >= 5000 || IsAScripter(playerid))
            {
	            new x_nr[16];
				new giveplayerid;
			    ...
			    if(strcmp(x_nr,"auto",true) == 0)
				{
				    if(PlayerInfo[playerid][pRank] >= 1 || PlayerInfo[playerid][Admin] >= 5000 || IsAScripter(playerid))
		            {
						...
						{
						    if(PlayerInfo[giveplayerid][pCarLic] == 3 || PlayerInfo[playerid][Admin] >= 5000 || IsAScripter(playerid))
						    {
