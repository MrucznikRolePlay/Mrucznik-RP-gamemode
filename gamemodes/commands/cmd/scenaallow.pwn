...
{
    if(PlayerInfo[playerid][Admin] < 200 ) return 1;
    new id;
    if(sscanf(params, "k<fix>", id)) return sendTipMessage(playerid, "U¿yj /scenaallow [Nick/ID]");
    if(GetPVarInt(id, "scena-req") != 2 && PlayerInfo[playerid][Admin] < 1) return sendTipMessageEx(playerid, COLOR_GRAD2, "Ten gracz nie prosi³ o to.");
    new str[128];
