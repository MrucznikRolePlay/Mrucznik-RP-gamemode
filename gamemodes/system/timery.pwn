	...
	foreach(new j : Player)
	{
		if(gPlayerLogged[j] > 0 && GetPlayerSkin(j) == 0 && GetPlayerAdminDutyStatus(j) == 0 && GetPVarInt(j, "JestPodczasWjezdzania") == 0 && GetPVarInt(j, "IsAGetInTheCar") == 0)
		{
			...
			else 
			{
				...
				sendTipMessage(j, "Posiada³eœ skin CJ-a ID [0] - przywróciliœmy Ci domyœlny skin. Je¿eli uwa¿asz ¿e to b³¹d, zg³oœ utratê w dziale b³êdów.");
				sendTipMessage(j, "[.] opisz co siê sta³o, np. dosta³eœ unfrakcjê lub jesteœ w rodzinie, frakcji. Pomocna bêdzie ka¿da informacja.");
			}
		}
	}
}
