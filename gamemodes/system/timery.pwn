		...
		if(RingTone[i] > 0 && Mobile[i] >= 0)
		{
			...
			else
			{
				...
				if(RingTone[i]%12 == 0 || RingTone[i] == 1)
				{
					...
					if(slotKontaktu >= 0)
					{
						format(string, sizeof(string), "TwÛj telefon dzwoni, (aby odebraÊ wpisz: /od) dzwoniπcy: %s (%d)", Kontakty[i][slotKontaktu][eNazwa], PlayerInfo[caller][pPnumber]);
					}
					else
					{
						format(string, sizeof(string), "TwÛj telefon dzwoni, (aby odebraÊ wpisz: /od) dzwoniπcy: %d", PlayerInfo[caller][pPnumber]);
					}
