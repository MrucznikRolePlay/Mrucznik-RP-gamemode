...
public CheckCode2003(killerid, playerid)
{
	...
	{
	    format(string, sizeof string, "ACv2 [#2003] WARNING: Prawdopodobnie prÛba wymuszenia kodu na graczu %s.", GetNickEx(killerid));
    	SendCommandLogMessage(string);
		Log(warningLog, INFO, string);
	}
}
...
public PlayerAFK(playerid, afktime, breaktime)
{
	if(IsPlayerPaused(playerid))
	{
		...
		if(afktime == 840 && GetPlayerAdminDutyStatus(playerid) == 1)
		{
			...
			SendClientMessage(playerid, COLOR_PANICRED, "Za minutÍ zostaniesz wyrzucony za Anty-AFK.");
		}
		...
		if(afktime > 600 && (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pHalfAdmin] >= 1 || IsAScripter(playerid)))
		{
			...
			else
			{
				...
				{
					SendClientMessage(playerid, 0xAA3333AA, "Nie wolno afczyÊ podczas @Duty! Otrzymujesz Kicka za AFK (15min)");
					SetTimerEx("KickEx", 500, false, "i", playerid);
				}
				SetPlayerChatBubble(playerid, caption, 0x33AA33AA, 20.0, 1500);
			}
    ...
    foreach(new i : Player)
	{
		...
		if(PlayerInfo[i][pJailed] > 0)
		{
			...
			if(PlayerInfo[i][pJailed] == 3)
			{
				...
				{
					sendErrorMessage(i, "Nie znajdujesz siÍ w AJ! Pozycja przywrÛcona do poprawnej!");
					SetPlayerSpawn(i); 
				}
			}
		...
		if(gPlayerLogged[j] > 0)
		{
			...
			if(GetPlayerSkin(j) == 0 && GetPlayerAdminDutyStatus(j) == 0 && GetPVarInt(j, "JestPodczasWjezdzania") == 0 && GetPVarInt(j, "IsAGetInTheCar") == 0)
			{
				...
				else 
				{
					...
					sendTipMessage(j, "Posiada≥eú skin CJ-a ID [0] - przywrÛciliúmy Ci domyúlny skin. Uwaøasz, øe to b≥πd? Zg≥oú utratÍ w dziale b≥ÍdÛw.");
					sendTipMessage(j, "[.] opisz dok≥adnie co siÍ sta≥o, np. dosta≥eú unfrakcjÍ lub jesteú w rodzinie, frakcji. Pomocna bÍdzie kaøda informacja.");
				}
			}
		}
	}
...
public CarCheck()
{
	...
	foreach(new j : Player)
	{
        if(kaska[j] < 0)
		{
			if(MoneyMessage[j]==0)
			{
				format(string, sizeof(string), "Masz d≥ugi, musisz zarobiÊ do nastÍpnej wyp≥aty $%d inaczej naúlemy na ciebie policje.",PlayerInfo[j][pBankMoney]);
