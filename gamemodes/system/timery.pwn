...
public ActorsFix(playerid)
{
	...
	{
		sendTipMessage(playerid, "Reset AktorÛw - UDANY"); 
	}
...
public SetTimeAndWeather(playerid)
{
	...
	sendTipMessage(playerid, "Pomyúlnie ustalono pogodÍ i czas dla VW"); 
...
public glosuj_admin_ankieta()
{
	...
	format(string, sizeof(string), "Uøytkownicy, ktÛrzy g≥osowali na TAK: %d", glosowanie_admina_tak);
	SendAdminMessage(COLOR_P@, string);
	format(string, sizeof(string), "Uøytkownicy, ktÛrzy g≥osowali na NIE: %d", glosowanie_admina_nie);
	SendAdminMessage(COLOR_P@, string);
...
public KomunikatCzas(playerid)
{
	...
	if(komunikatMinuty[playerid] == 15)
	{
		...
		sendTipMessage(playerid, "ZakoÒczono odliczanie - Moøesz ponownie wys≥aÊ komunikat frakcyjny"); 
...
public KomunikatCzasZerowanie(playerid)
{
	...
	if(komunikatMinutyZerowanie[playerid] == 5)
	{
		new string[128];
		format(string, sizeof(string), "null"); 
		sendTipMessage(playerid, "ZakoÒczono odliczanie - Moøesz ponowie wys≥aÊ komunikat frakcyjny"); 
		SetPVarString(playerid, "trescOgloszenia", string);
	...
	if(IsPlayerPaused(playerid))
	{
		...
		if(afktime > 600 && PlayerInfo[playerid][pAdmin] >= 1 ||afktime > 600 && PlayerInfo[playerid][pNewAP] >= 1)
		{
			...
			else
			{
				...
				{
					SendClientMessage(playerid, 0xAA3333AA, "Nie wolno afczyÊ podczas wejúcia na @Duty! Otrzymujesz Kicka (10min)");
					SetTimerEx("KickEx", 500, false, "i", playerid);
				}
				SetPlayerChatBubble(playerid, caption, 0x33AA33AA, 20.0, 1500);
			}
				...
				if(playerState == 1 || playerState == 2 || playerState == 3)
				{
					if(GetPVarInt(i, "dutyadmin") == 0)
					{
						...
						switch(weaponID)
						{
							...
							case 2:
							{
								format(string, sizeof(string), "* %s wyciπga Kij Golfowy.", specNAME);
								ProxDetector(10.0, i, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
							}
...
public PlayerShowForHunter(playerid)
{
	...
	if(hunterStatus[playerid] == 1 || hunterStatus[playerid] == 6 || hunterStatus[playerid] == 18)//Aktualizacja 
	{
		...
		format(string, sizeof(string), "IloúÊ osÛb z Wanted Level +10 to %d", wantedValuePlayer); 
		sendTipMessage(playerid, string);
		sendTipMessage(playerid, "Pomyúlnie zaktualizowano po≥oøenia na GPS"); 
    ...
    foreach(new i : Player)
	{
		...
		if(PlayerInfo[i][pJailed] > 0)
		{
			...
			if(PlayerInfo[i][pJailTime] <= 0 && WantLawyer[i] == 0)
			{
				...
				else if(PlayerInfo[i][pJailed] == 3)
				{
					...
					{
						format(string, sizeof(string), "[Marcepan Marks] Zabra≥em graczu %s broÒ [Odsiedzia≥ karÍ za DM2]", GetNick(i, true));
						SendAdminMessage(COLOR_PANICRED, string);
						format(string, sizeof(string), "%s zabra≥em twojπ broÒ. Z pozdrowieniami - Marcepan Marks", GetNick(i, true));
		...
		if(TutTime[i] >= 1 && !IsPlayerNPC(i))
		{
			...
			if(TutTime[i] == 3)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Skoro juø wiesz, co to jest Role Play musisz poznaÊ zasady panujπce na naszym serwerze.");
				SendClientMessage(i, COLOR_WHITE, "W tym celu przejdziesz teraz drobny samouczek tekstowy, ktÛry przygotuje CiÍ do rozgrywki!"); 
			}
			...
			else if(TutTime[i] == 114)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Zapewne masz jeszcze sporo pytaÒ dotyczπcych gry. Spokojnie, znajdziesz na nie odpowiedü!");
				SendClientMessage(i, COLOR_WHITE, "Moøesz úmia≥o pytaÊ administratora (/admins), poprzez zapytania (/zapytaj), bπdü teø [.]");
				SendClientMessage(i, COLOR_WHITE, "[.] poprzez chat dla nowych graczy /newbie. To juø koniec samouczka. ");
				SendClientMessage(i, COLOR_WHITE, "Zasad, poradnikÛw i pomocy jest znacznie wiÍcej na naszym forum! Odwiedü je: https://mrucznik-rp.pl");
			}
