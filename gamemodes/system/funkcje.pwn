	...
	foreach(new i2 : Player)
	{
		if(status == 1)
		{
			if(winners == 0)
			{
				...
				format(string, sizeof(string), "Nagroda zosta≥a podwyøszona do %d$", Jackpot);
				OOCOff(COLOR_WHITE, string);  
				break; 
			}
		}
...
RemoveLeadersFromFraction(giveplayerid, playerid)//Usuwa liderÛw frakcji - giveplayerid to lider, playerid to osoba nadajπca
{
	...
	format(query, sizeof(query), "UsuniÍto wszystkich liderÛw z frakcji %s", FractionNames[GetPlayerFraction(giveplayerid)]);
	...
	Log(adminLog, INFO, "Admin %s usunπ≥ gracza %s z frakcji %d - pozostawiajπc VLD.", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), PlayerInfo[giveplayerid][pMember]);
