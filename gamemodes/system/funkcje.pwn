...
RemoveLeadersFromFraction(giveplayerid, playerid)//Usuwa liderÛw frakcji - giveplayerid to lider, playerid to osoba nadajπca
{
	...
	Log(adminLog, INFO, "Admin %s usunπ≥ gracza %s z frakcji %d - Usuwajπc VLD", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), PlayerInfo[giveplayerid][pMember]);
	...
	format(string, sizeof(string), "  Wyrzuci≥es %s z frakcji - usuwajπc przy tym wszystkich  VLD", GetNick(giveplayerid));
	...
	for(new i; i<MAX_LEADERS+1; i++)
	{
		...
		if(liderFraction[i] == frac_Gracza)
		{
			...
			mysql_query(queryS);
			format(string, sizeof(string), "%s zosta≥ usuniÍty z frakcji - posiada≥ VLD", liderName);
