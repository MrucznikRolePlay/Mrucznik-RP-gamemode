...
ToggleInwigilacja(playerid, adminid)
{
	if(PlayerInfo[playerid][pPodPW] == 0)
	{
		...
		SendCommandLogMessage(sprintf("Admin %s [%d] w≥πczy≥ inwigilacje dla %s [%d]", GetNick(adminid), adminid, GetNick(playerid), playerid));
	...
	else if(PlayerInfo[playerid][pPodPW] == 1)
	{
		...
		SendCommandLogMessage(sprintf("Admin %s [%d] wy≥πczy≥ inwigilacje dla %s [%d]", GetNick(adminid), adminid, GetNick(playerid), playerid));
    ...
    if(IsPlayerConnected(taxidriver))
	{
	    ...
	    if(PlayerInfo[playerid][pLevel] < 3)
	    {
			if(kaska[playerid] < floatround(TransportCost[playerid]*0.25))
			{
				...
				SetPlayerCriminal(playerid,INVALID_PLAYER_ID, "Kradzieø taksÛwkarza(brak pieniÍdzy na sp≥ate)");
...
public ZestawNaprawczy_CountDown(playerid, vehicleid)
{
	...
	if (ZestawNaprawczy_Timer[playerid] > 0)
	{
		...
		{
			format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~y~Pozosta≥o do koÒca naprawy: %ds", ZestawNaprawczy_Timer[playerid]);
		...
		{
			format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~r~Podejdü do auta! %ds", 8-ZestawNaprawczy_Warning[playerid]);
			GameTextForPlayer(playerid, string, 2500, 3);
...
public naczasbicie(playerid, playerid_atak){
	...
	SendClientMessage(playerid_atak, COLOR_NEWS, "Wygra≥eú bitwÍ poniewaø broniπcy za d≥ugo wpisywa≥ znaki!");
...
UnFrakcja(playerid, para1, bool:respawn = true)
{
	...
	{
		format(string, sizeof(string), "%s jest g≥Ûwnym liderem organizacji - czy chcesz zwolniÊ\nWSZYSTKICH liderÛw z organizacji? (Zabierze VLD)", GetNick(para1));
	...
	Log(adminLog, INFO, "Admin %s usunπ≥ gracza %s z frakcji %d", GetPlayerLogName(playerid), GetPlayerLogName(para1), PlayerInfo[para1][pMember]);
...
Oil_Destroy(lID)
{
    ...
    foreach(new i : Player)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[ERS] Usunπ≥eú plamÍ oleju! Otrzymujesz 7 500$! [ERS]");
        DajKase(i, 7500);
        SendFamilyMessage(4, COLOR_GREEN, "[ERS] Straøak usunπ≥ plamÍ oleju! Na konto frakcji wp≥ywa 12 500$! [ERS]");
...
public DeathAdminWarning(playerid, killerid, reason)
{
	...
	if(killerid != INVALID_PLAYER_ID)
	{
		...
		{
			//-------<[  Logi  ]>---------
			Log(warningLog, INFO, "%s %s %s z broni o id %d bÍdπc w aucie (moøliwe DB/CK2).", GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid), reason);
			...
			{
				format(string, sizeof(string), "AdmWarning: %s[%d] %s %s[%d] bÍdπ w aucie (moøliwe DB/CK2) [GunID %d]!", killername, killerid, playername, bwreason, playerid, reason);
		...
		else
		{
			switch(reason)
			{
				...
				case 41:
				{
					...
					SendMessageToAdmin(string, COLOR_YELLOW);
					Log(warningLog, INFO, "%s %s gracza %s uøywajπc spray'a", GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid));
					SendMessageToAdminEx(string, COLOR_P@, 2);
				}
				default:
				{
					...
					if(GetPVarInt(playerid, "skip_bw")  == 0)
					{
						if(PlayerInfo[playerid][pInjury] > 0)
						{
							...
							{
								format(string, sizeof(string), "AdmWarning: £owca NagrÛd [%d]%s %s gracza %s bez oferty /poddajsie !", killerid, killername, bwreason, playername);
								SendMessageToAdmin(string, COLOR_YELLOW);
								Log(warningLog, INFO, "£owca nagrÛd %s %s gracza %s bez oferty /poddajsie", GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid));
							}
...
public UnCuffedAction(playerid, cuffedid)
{
	...
	GetPlayerName(playerid, sendername, sizeof(sendername));
	format(string, sizeof(string), "* Zosta≥eú rozkuty przez %s.", sendername);
	SendClientMessage(cuffedid, COLOR_LIGHTBLUE, string);
	format(string, sizeof(string), "* Rozku≥eú %s.", giveplayer);
	SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
