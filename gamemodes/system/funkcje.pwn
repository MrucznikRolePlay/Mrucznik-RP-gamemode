...
Taxi_FareEnd(playerid)
{
	...
	format(string, sizeof(string), "Zakoñczy³e s³u¿bê, zarobi³e $%d dla Korporacji Transportowej", TransportMoney[playerid]);
...
public Dopalaj(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		{
			SendClientMessage(playerid, TEAM_CYAN_COLOR, "Dopalacz deaktywowany - zbyt ma³a prêdkoæ");
			return 1;
		}
	}
	else
...
public ZestawNaprawczy_CountDown(playerid, vehicleid)
{
	...
	if (ZestawNaprawczy_Timer[playerid] > 0)
	{
		...
		{
			SendClientMessage(playerid, COLOR_PANICRED, "* [ZESTAW NAPRAWCZY] Anulowano naprawê pojazdu. Nie by³e w dostatecznie blisko pojazdu.");
			ZestawNaprawczy_Timer[playerid] = 30;
			ZestawNaprawczy_Warning[playerid] = 0;
			DeletePVar(playerid, "Use_ZestawNaprawczy");
		}
...
public TiNzPJwGUI(playerid)
{
	...
	format(string, sizeof(string), "* S³ychaæ dwiêk koñca czasu na rozwi¹znie testu ((%s))", playername);
...
public zabezpieczeniewoz(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
	    SendClientMessage(playerid, COLOR_PANICRED, "Dosta³e kicka za prawdopodobieñstwo u¿ycia czitów. Jeli to bug, napisz na forum: www.mrucznik-rp.net");
		KickEx(playerid);
	}
	return 1;
}*/
...
public naczasbicie(playerid, playerid_atak){
	...
	SendClientMessage(playerid_atak, COLOR_NEWS, "Wygra³e bitwê poniewa¿ broni¹cy za d³ugo wpisywa³ znaki!");
...
/*public UzyteKajdany(playerid,giveplayerid)
{
	if(Kajdanki_PDkuje[playerid] > 0 && PlayerInfo[giveplayerid][pJob] == 1)
	{
		...
		ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
 		format(string, sizeof(string), "Sku³e %s. Masz 2 minuty, by dostarczyæ go do celi!", GetNick(playerid));
...
poscig[playerid] = 0;
SendClientMessage(playerid,COLOR_PANICRED,"|_________________Tryb Pocigu ZAKOÑCZONY_________________|");
SendClientMessage(playerid,COLOR_WHITE,"Czas na z³apanie ciê dobieg³ koñca.");
SendClientMessage(playerid,COLOR_WHITE,"Je¿eli nie przeprowadzasz ju¿ ¿adnej akcji RP, mo¿esz bezpiecznie wyjæ z gry");
...
if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
{
	...
	format(komunikat, sizeof(komunikat),"* %s ³¹czy kabelki i wyjmuje rubokrêt i odkrêca nastêpn¹ os³onkê.", nick);
...
//odpali³
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
    	...
    	{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 2, masz teraz wiêksze szanse ¿e uda ci siê ukraæ wóz."); }
    	else if(PlayerInfo[playerid][pJackSkill] == 100)
    	{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 3, masz teraz wiêksze szanse ¿e uda ci siê ukraæ wóz."); }
    	else if(PlayerInfo[playerid][pJackSkill] == 200)
    	{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 4, masz teraz wiêksze szanse ¿e uda ci siê ukraæ wóz."); }
    	else if(PlayerInfo[playerid][pJackSkill] == 400)
    	{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 5, masz teraz wiêksze szanse ¿e uda ci siê ukraæ wóz."); }
    }
    else
    {
        NieSpamujKradnij[playerid] = 0;
...
//alarm siê w³¹cza
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
    	...
    	GetPlayerName(playerid, nick, sizeof(nick));
    	format(komunikat, sizeof(komunikat),"* %s le ³¹czy kabelki po czym w³¹cza siê alarm.", nick);
    	ProxDetector(20.0, playerid, komunikat, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
KoniecWyscigu(playerid)
{
	new string[256];
	if(playerid == -1)
		format(string, sizeof(string), "Komunikat wycigowy: {FFFFFF}Wyscig %s zakoñczony przez system.", Wyscig[Scigamy][wNazwa]);
	else if(playerid == -2)
		format(string, sizeof(string), "Komunikat wycigowy: {FFFFFF}Wyscig %s zakoñczony - wszyscy dojechali do mety!", Wyscig[Scigamy][wNazwa]);
	else
		format(string, sizeof(string), "Komunikat wycigowy: {FFFFFF}Wyscig %s zakoñczony przez %s.", Wyscig[Scigamy][wNazwa], GetNickEx(playerid));
...
PrawoJazdyRandomGUITest(playerid, questions_available[], questions_count)
{
    ...
    new questions_content[][] =
    {
        ...
        "Ile wynosi dopuszczalna prêdkoæ\nprzy holowaniu w terenie zabudowanym?\n\nPodaj w cyfrach",
        "Któr¹ stron¹ jezdni nale¿y jechaæ?\n\nPodaj jeden wyraz",
        "Co nale¿y rozstawiæ po wypadku?\n\nPodaj jeden wyraz",
        "Dozwolona prêdkoæ na autostradzie?\n\nPodaj w cyfrach",
        "Dozwolona prêdkoæ na miecie?\n\nPodaj w cyfrach",
        "Jaka jest maksymalna, dozwolona\nprêdkoæ na drodze szybkiego ruchu?\n\nPodaj w cyfrach",
			        ...
			        foreach(new k : Player)
					{
						if(IsPlayerConnected(k))
						{
				        	if(PlayerInfo[k][pMember] == 8 || PlayerInfo[k][pLider] == 8)
				        	{
				                ...
				                SendClientMessage(k, COLOR_DBLUE, "*** Wiadomoæ: Nowe zlecenia s¹ ju¿ dostêpne. ***");
...
UsunBron(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		PlayerInfo[playerid][pGun0] = 0;//Piêæ lub kastet
...
WejdzInt(playerid, Float:x, Float:y, Float:z, Float:x2, Float:y2, Float:z2, Float:tolerancja, interior, vw, komunikat[]="", local, gametext[]="")
{
    ...
    {
		if(x==x2 && y==y2 && z==z2) return sendErrorMessage(playerid, "Nie mo¿na tutaj wejæ.");
		//Komunikaty funkcji:
...
WyjdzInt(playerid, Float:x, Float:y, Float:z, Float:x2, Float:y2, Float:z2, Float:tolerancja, interior, vw, komunikat[]="", local)
{
    ...
    if(GetPlayerVirtualWorld(playerid) == vw && GetPlayerInterior(playerid) == interior && IsPlayerInRangeOfPoint(playerid, tolerancja, x, y, z))
    {
		if(vw == 55)// KASYNO 55
		{
			if(GetPVarInt(playerid, "kostka") == 1) return sendTipMessageEx(playerid, COLOR_PANICRED, "Nie mo¿esz wyjæ z kasyna bêd¹c w trakcie gry!");
	...
	foreach(new i2 : Player)
	{
		...
		if(status == 2)
		{
			if(winners == 0)
			{
				...
				Jackpot = rand; 
				format(string, sizeof(string), "Lotto rozpoczyna kolejn¹ loteriê, kasa wyjcia to %d$", Jackpot); 
				OOCOff(COLOR_WHITE, string); 
				status = 1;
				break;
			}
...
SetPlayerCriminal(playerid,declare,reason[], bool:sendmessage=true)
{
	if(IsPlayerConnected(playerid))
	{
		...
		format(turnmes, sizeof(turnmes), "Pope³ni³e przestêpstwo ( %s ). Zg³osi³: %s.",reason,turner);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "Zabiæ:[%d] mierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Rodzina:[%s] Skin ID:[%d]",kills,deaths,costlevel,exp,expamount,wanted,f2text, skin);
...
orgInvitePlayer(playerid, orguid)
{
    ...
    new lStr[64];
    format(lStr, 64, "ORG » Zosta³e przyjêty do organizacji %s.", OrgInfo[orgid][o_Name]);
...
orgGivePlayerRank(playerid, callerid, rankid)
{
    ...
    if(!orgIsValid(orgid)) return 0;
    new lStr[128];
    format(lStr, 128, "ORG » Otrzyma³e %d rangê (%s) w organizacji %s. Nada³: %s.", rankid, (strlen(FamRang[PlayerInfo[playerid][pOrg]][rankid]) > 1) ? (FamRang[PlayerInfo[playerid][pOrg]][rankid]) : (FamRang[0][rankid]), OrgInfo[orgid][o_Name], (callerid == INVALID_PLAYER_ID) ? ("SYSTEM") : (GetNick(callerid)));
    SendClientMessage(playerid, COLOR_LIGHTBLUE, lStr);
...
Dom_ChangeOwner(playerid, dom, forid)
{
	...
	Log(adminLog, INFO, "Admin %s zmieni³ w³aciciela domu %d z %s na %s", GetPlayerLogName(playerid), dom, Dom[dom][hWlasciciel], GetPlayerLogName(forid));
	...
	else
	{
	    ...
	    if(platnosc == 2)
		{
		    if(PlayerInfo[playerid][pAccount] >= cenadomu && cenadomu != 0 && cenadomu > 0 && PlayerInfo[playerid][pAccount] != 0 && PlayerInfo[playerid][pAccount] > 0)
	        {
				...
				format(str2, sizeof(str2), "Gratulacje! Kupi³e dom za %d$. Zosta³o one odliczone od twojego konta bankowego które wynosi teraz %d", cenadomu, PlayerInfo[playerid][pAccount]);
...
ZlomowanieDomu(playerid, dom)
{
	...
	if(dini_Exists(plik))
	{
		...
		if(playerid != 9999)
		{
			...
			new GeT2[512];
			format(GeT2, sizeof(GeT2), "Sprzeda³e swój dom za %d$. Osoby wynajmuj¹ce zosta³y wyeksmitowane. Przedmioty w sejfie oraz dodatki do domu przepad³y.", (Dom[dom][hCena]/2));
	...
	{
	    SendClientMessage(playerid, COLOR_NEWS, "B£AD - Twój dom nie istnieje, zg³o to na forum !!");
	}
    return 1;
}
...
NaprawSpojnoscWlascicielaDomu(playerid)
{
	...
	if(domcheck == 1)
	{
		if(Dom[dom][hKupiony] == 0)
		{
			Dom[dom][hKupiony] = 1;
			ZapiszDom(dom);
			SendClientMessage(playerid, COLOR_PANICRED, "Wykryto bug z niekupionym domem, zosta³ on automatycznie naprawiony. Je¿eli komunikat bêdzie siê powtarza³ lub wyst¹pi¹ inne bugi, zg³o to koniecznie na forum!");
	...
	else if(domcheck == 0)
	{
		...
		{
			DajKase(playerid, (Dom[dom][hCena]/2));
			SendClientMessage(playerid, COLOR_PANICRED, "Twój dom zosta³ zabrany z powodu nieaktywnoci, otrzymujesz po³owê wartosci domu!");
		...
		{
			DajKase(playerid, (Dom[dom][hCena]/2));
			SendClientMessage(playerid, COLOR_PANICRED, "Wykro bug z dwoma w³acicielami! Jeste drugim w³acicielem, wiêc tracisz dom, otrzymujesz po³owê wartosci domu.");
			Log(payLog, INFO, "%s straci³ dom %s z powodu nieaktywnoci (2 w³acicieli) i otrzyma³ %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
	}
	else
	{
		SendClientMessage(playerid, COLOR_PANICRED, "Twój dom nie istnieje! Prawdopodobnie zosta³ usuniêty lub co posz³o nie tak.");
...
KupowanieDodatkow(playerid, dom)
{
    ...
    ShowPlayerDialogEx(playerid, 826, DIALOG_STYLE_LIST, "Kupowanie dodatków", strALL, "Wybierz", "Wyjd");
    return 1;
}
...
{
    new charsets[46] = "0123456789a¹bcædeêfghijkl³mnñoóprstuvwyz¿xq";
...
Oil_Destroy(lID)
{
    ...
    foreach(new i : Player)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[ERS] Usun¹³e plamê oleju! Otrzymujesz 7 500$! [ERS]");
        DajKase(i, 7500);
...
ZaladujSamochodyDoKradziezy()
{
    ...
    for(new i = 0; i < 183; i++)
	{
		if(i >= 165) // Faggio dla z³odziejów aut w dziuplach - zawsze ma byæ okrelony model
		{
...
BuyDrinkOnClub(playerid, const dName[], dCost, dLVL, dAction)
{
	...
	if(GetPVarInt(playerid, "jestPrzyBarzeVIP") == 1)
	{
		...
		{
			sendErrorMessage(playerid, "Nie masz takiej iloci gotówki!"); 
			return 1;
		}
	}
...
ShowPersonalization(playerid, value)
{
	...
	if(value == 4)
	{
		...
		{
			strdel(persona_B, 0, 64); 
			strins(persona_B, "Wywietlanie nicków\t{80FF00}ON\n", 0);
		}
		else if(PlayerPersonalization[playerid][PERS_NICKNAMES] == 1)
		{
			strdel(persona_B, 0, 64);
			strins(persona_B, "Wywietlanie nicków\t{FF6A6A}OFF\n",0);
		}
...
public OnPlayerTakeDamageWeaponHack(playerid, weaponid, fakekillid)
{
	...
	{
		new string[128];
		format(string, sizeof string, "ACv2 [#2002]: %s mo¿e mieæ weapon hack. | Jeli fakekill, to: %s .", GetNickEx(playerid), GetNickEx(fakekillid));
		SendCommandLogMessage(string);
...
PursuitMode(playerid, giveplayerid)
{
	if(ProxDetectorS(80.0, playerid, giveplayerid))
	{
		if(poscig[giveplayerid] != 1)
		{
			...
			SendClientMessage(giveplayerid,COLOR_WHITE,"S³u¿by porz¹dkowe ruszy³y za tob¹ w pocig! W takim wypadku najlepiej siê poddaæ!");
			SendClientMessage(giveplayerid,COLOR_WHITE,"W trybie pocigu nie mozesz wyjæ z gry, zgin¹æ oraz byæ AFK.");
	...
	{
		sendErrorMessage(playerid, "Gracz jest za daleko by nadaæ mu tryb pocigu.");
	}
}
...
// wyznaczenie wierzcho³ków prostopad³ocianu, który wyznacza granice pojazdu
