	...
	foreach(new i : Player)
	{
		if(IsPlayerConnected(i))
		{
			if(PlayerInfo[i][LottoNr] > 0)
			{
				...
				{
					format(string, sizeof(string), "Niestety nie uda≥o Ci siÍ wygraÊ loterii z numerem %d.", PlayerInfo[i][LottoNr]);
					SendClientMessage(i, COLOR_WHITE, string);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		new atext[20];
		if(PlayerInfo[targetid][pGender] == 1) { atext = "MÍøczyzna"; }
	...
	else
	{
	    ...
	    if(platnosc == 2)
		{
		    if(PlayerInfo[playerid][pBankMoney] >= cenadomu && cenadomu != 0 && cenadomu > 0 && PlayerInfo[playerid][pBankMoney] != 0 && PlayerInfo[playerid][pBankMoney] > 0)
	        {
				...
				format(str2, sizeof(str2), "Gratulacje! Kupi≥eú dom za %d$. Zosta≥o one odliczone od twojego konta bankowego ktÛre wynosi teraz %d", cenadomu, PlayerInfo[playerid][pBankMoney]);
...
GPSMode(playerid, bool:red = false)
{
	...
	if(red)
	{
		format(string, sizeof(string), "=: %s %s %s GPS %s :=", FracRang[GetPlayerFraction(playerid)][PlayerInfo[playerid][Rank]], sendername, (PDGPS == playerid) ? ("deaktywowa≥") : ("aktywowa≥"), (PDGPS == playerid) ? ("- odwo≥ujÍ RED") : ("potrzebne wsparcie! - CODE RED"));
