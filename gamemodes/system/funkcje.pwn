...
{
	if(PlayerInfo[playerid][pPodPW] == 0 && NISKI_ADMIN_LVL == true) return sendTipMessage(playerid, "W≥πczanie od 150@lvl");
				...
				foreach(new i : Player)
				{
					if(IsPlayerConnected(i))
					{
					    if(IsACop(i))
					    {
					        if(gCrime[i] == 0)
					        {
								format(cbjstore, sizeof(turnmes), "HQ: PrzestÍpstwo: %s",reason);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		if(PlayerInfo[targetid][pBusinessOwner] != INVALID_BUSINESSID)
		{
			...
			strcat(bizranga, "W≥aúciciel", sizeof(bizranga));  
		}
