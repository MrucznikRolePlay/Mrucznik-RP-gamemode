...
Taxi_FareEnd(playerid)
{
	...
	if(GetPlayerFraction(playerid) != FRAC_KT) 
	{
		...
		Log(payLog, INFO, "%s zarobi≥ $%d na s≥uøbie taksÛwkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
	}
    else
    {
        Sejf_Add(FRAC_KT, TransportMoney[playerid]);
		Log(payLog, INFO, "%s zarobil $%d dla KT na s≥uøbie taksÛwkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
    }
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		if(PlayerInfo[targetid][pPbiskey] >= 0 && PlayerInfo[targetid][pPbiskey] <= MAX_BIZNES)
		{
			...
			format(coordsstring, sizeof(coordsstring), "Biznes:[%s] MaxDochÛdBiz[%d] BizID [%d]", BizData[bizid][eBizName], BizData[bizid][eBizMoney], bizid);
			SendClientMessage(playerid, COLOR_GRAD5, coordsstring);
		}
...
orgPrintType(lID) return OrgTypes[orgType(lID)];
...
Dom_ChangeOwner(playerid, dom, forid)
{
	...
	Log(adminLog, INFO, "Admin %s zmieni≥ w≥aúciciela domu %d z %s na %s", GetPlayerLogName(playerid), dom, Dom[dom][hWlasciciel], GetPlayerLogName(forid));
...
NaprawSpojnoscWlascicielaDomu(playerid)
{
	...
	else if(domcheck == 0)
	{
		...
		else
		{
			...
			Log(payLog, INFO, "%s straci≥ dom %s z powodu nieaktywnoúci (2 w≥aúcicieli) i otrzyma≥ %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
	}
	else
...
enjoter(color,const string[], level)
{
	foreach(new i : Player)
	{
		if(IsPlayerConnected(i))
...
Niwelacje(playerid)
{
	//TODO usunπc albo poprawiÊ, bo rzygaÊ siÍ chce jak na to patrzÍ
...
stock StartTrashCollecting(playerid)
{
	if(pTrashCollected[playerid] <= 5)//Pierwsza strefa
	{
		...
		sendTipMessageEx(playerid, COLOR_RED, "Na tej strefie znajdziesz 5 úmietnikÛw");
	}
	return 1;
}
