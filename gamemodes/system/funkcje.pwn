...
ToggleInwigilacja(playerid, adminid)
{
	if(PlayerInfo[playerid][pPodPW] == 0)
	{
		...
		SendCommandLogMessage(sprintf("Admin %s [%d] w≥πczy≥ inwigilacje dla %s [%d]", GetNick(adminid, true), adminid, GetNick(playerid), playerid));
	...
	else if(PlayerInfo[playerid][pPodPW] == 1)
	{
		...
		SendCommandLogMessage(sprintf("Admin %s [%d] wy≥πczy≥ inwigilacje dla %s [%d]", GetNick(adminid, true), adminid, GetNick(playerid), playerid));
