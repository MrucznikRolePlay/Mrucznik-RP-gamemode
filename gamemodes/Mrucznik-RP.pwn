...
public OnEnterExitModShop(playerid, enterexit, interiorid)
{
    ...
    if(enterexit == 0)
    {
        SendClientMessage(playerid, COLOR_WHITE, "Zap�aci�e� 2500$ za napraw� wozu.");
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if (IsACopCar(vehicleid) && !ispassenger)
	{
		if(IsACop(playerid))
		{
			SendClientMessage(playerid, COLOR_BLUE, "Po��czy�e� si� z komputerem policyjnym, wpisz /mdc aby zobaczy� kartotek� policyjn�");
		}
		else
		{
			new Float:slx, Float:sly, Float:slz;
	...
	if (IsAnAmbulance(vehicleid) && !ispassenger)
	{
		...
		{
			SendClientMessage(playerid, COLOR_GRAD1, "Nie jeste� medykiem na s�u�bie!");
		}
		else if(!IsAMedyk(playerid))
		{
			SendClientMessage(playerid, COLOR_GRAD1, "Nie jeste� medykiem!");
		}
	}
	if (GetVehicleModel(vehicleid) == 525)
	{
		SendClientMessage(playerid, COLOR_BROWN, "Wsiad�e� do holownika, naci�nij CTRL alby podholowa� w�z.");
	}
	#if DEBUG == 1
		printf("%s[%d] OnPlayerEnterVehicle - end", GetNick(playerid), playerid);
	#endif
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(PDGPS == playerid) //wy��czenie GPS
	{
		cmd_gps(playerid);
	}
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(IsPlayerConnected(playerid) && playerid != INVALID_PLAYER_ID)
	{
		...
		{
			lowcaz[playerid] = 501;
			SendClientMessage(playerid, COLOR_YELLOW, "Zlecenie zosta�o anulowane - nie mo�esz wzia�� teraz zlecenia na tego samego gracza!");
		}
		...
		else
		{
		    if(PlayerInfo[playerid][pSpawn] == 0) //Normalny spawn
		    {
				...
				else if(GetPlayerOrg(playerid) > 0) //Spawn Organizacji
				{
		            ...
		            {
						SendClientMessage(playerid, COLOR_YELLOW, "Twoja rodzina nie ma jeszcza spawna - spawnujesz si� jako cywil.");
	...
	if(ScigaSie[playerid] != 666 && IloscCH[playerid] != 0 && Scigamy != 666)
	{
	    ...
	    else if(IloscCH[playerid] == Wyscig[Scigamy][wCheckpointy])
	    {
	        ...
	        if(Ukonczyl == 1)
	        {
		        ...
		        {
		        	format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s wygra� wy�cig %s", GetNick(playerid), Wyscig[Scigamy][wNazwa]);
		        	foreach(Player, i)
		        	{
	        ...
	        else
	        {
	            if(Ukonczyl >= IloscZawodnikow)
		        {
					...
					format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s uko�czy� wy�cig jako ostatni !", GetNick(playerid));
					WyscigMessage(COLOR_YELLOW, string);
		            KoniecWyscigu(-2);
		        }
		        else
		        {
					...
					format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s uko�czy� wy�cig jako %d !", GetNick(playerid), Ukonczyl);
					WyscigMessage(COLOR_YELLOW, string);
	            	Ukonczyl++;
		        }
	        }
...
public OnRconLoginAttempt(ip[], password[], success)
{
    ...
    else
    {
        if(player != -1)
        {
			...
			{
				new str[128];
				format(str, 128, "RCON: U�ytkownik %s (%d) pr�bowa� si� zalogowa� na rcona bez wymaganych uprawnie�!", GetNick(player), player);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	{// 38 / 49 / 56 = SS
		...
		if(IsABoat(newcar))
		{
			...
			{
			    SendClientMessage(playerid, COLOR_GREY, "   Nie wiesz w jaki spos�b p�ywa� �odzi�, wi�c decydujesz si� j� opu�ci� !");
			    RemovePlayerFromVehicleEx(playerid);
			}
		}
		else if(IsAPlane(newcar))
		{
		    ...
		    if(PlayerInfo[playerid][pFlyLic] < 1)
			{
				...
				{
					SendClientMessage(playerid, COLOR_GREY, "   Nie wiesz w jaki spos�b lata� samolotem, wi�c decydujesz si� go opu�ci� !");
					RemovePlayerFromVehicleEx(playerid);
				}
			}
		}
		else
		{
			if(PlayerInfo[playerid][pCarLic] != 1)
			{
				...
				{
					SendClientMessage(playerid, COLOR_GREY, "   Nie masz prawa jazdy, postanawiasz opu�ci� pojazd!");
					RemovePlayerFromVehicleEx(playerid);
				}
		...
		if(IsACopCar(newcar))
		{
		    if(IsACop(playerid))
		    {
		        if(OnDuty[playerid] == 0)
		        {
		            ...
		            {
		            	RemovePlayerFromVehicleEx(playerid);
		            	SendClientMessage(playerid, COLOR_GREY, "Musisz by� na s�u�bie aby je�dzi� autem policyjnym !");
						#if DEBUG == 1
							printf("%s[%d] OnPlayerStateChange - end", GetNick(playerid), playerid);
						#endif
                        return 1;
        ...
        if(newcar <= CAR_End) //do kradziezy
        {
		    ...
		    {
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Mo�esz ukra�� ten w�z, wpisz /kradnij spr�bowa� to zrobi� lub /wyjdz aby wyj��.");
        ...
        if(VehicleUID[newcar][vUID] != 0)
        {
            ...
            else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_SPECIAL) //specjalne
            {
                ...
                else if(CarData[lcarid][c_Owner] == CAR_ZUZEL)
                {
        		    ...
        		    {
        				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Nie jeste� zawodnikiem �u�lowym, zg�o� si� do administracji je�li chcesz nim zosta�.");
        				RemovePlayerFromVehicleEx(playerid);
        			}
                }
            }
            else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_JOB) //reszta do pracy
            {
                ...
                if(CarData[lcarid][c_Owner] == JOB_BUSDRIVER) SendClientMessage(playerid, COLOR_YELLOW, "SERVER: Wpisz /trasa aby rozpocz�� prac�");
            }
            ...
            else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_PLAYER) //Pojazdy graczy
            {
                if(IsCarOwner(playerid, newcar, true))
    	        {
    	       	...
    	       	{
    	       	    SendClientMessage(playerid, COLOR_NEWS, "Nie masz kluczy do tego pojazdu, a zabezpieczenia s� zbyt dobre aby� m�g� go ukra��.");
    	       		RemovePlayerFromVehicleEx(playerid);
    	       	}
            }
        }
...
OnPlayerRegister(playerid, password[])
{
	if(IsPlayerConnected(playerid))
	{
        ...
        if(CheckAlfaNumeric(password))
        {
            ...
            SendClientMessage(playerid, COLOR_PANICRED, "Zalecamy zmieni� has�o poprzez /zmienhaslo");
            SendClientMessage(playerid, COLOR_PANICRED, "Zalecamy zmieni� has�o poprzez /zmienhaslo");
            SendClientMessage(playerid, COLOR_PANICRED, "Zalecamy zmieni� has�o poprzez /zmienhaslo");
            SendClientMessage(playerid, COLOR_PANICRED, "Zalecamy zmieni� has�o poprzez /zmienhaslo");
            SendClientMessage(playerid, COLOR_PANICRED, string);
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has�o
		...
		if( result == 1 )
		{
			SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {33CCFF}Twoje konto zosta�o pomy�lnie przekonwertowane. �yczymy mi�ej gry.");
		...
		{
			SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {FF0000}Sprawd� wielkos� znak�w, podobny nick istnieje.");
			KickEx(playerid);
			return 1;
		}
		else if(result == 0)
		{
			SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {FF0000}B��d przy konwersji konta! Zg�o� okoliczno�ci na forum.");
			KickEx(playerid);
			return 1;
		}
...
public OnPlayerText(playerid, text[])
{
	...
	{
		SendClientMessage(playerid, TEAM_CYAN_COLOR, "Nie mo�esz m�wi� gdy� jeste� uciszony");
		return 0;
	}
	...
	if(Mobile[playerid] != 1255)
	{
		...
		if(Mobile[playerid] == 913)
		{
			...
			if ((strcmp("nie", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("nie")))
			{
				...
				SendClientMessage(playerid, COLOR_DBLUE, "Dzi�kujemy za zg�oszenie przest�pstwa");
			...
			if (IsPlayerConnected(badguy))
			{
			    if(badguy != INVALID_PLAYER_ID)
			    {
					...
					{
						SendClientMessage(playerid, COLOR_DBLUE, "LSPD HQ: Musisz poinformowa� kogo� innego. To jest linia ratunkowa");
						SendClientMessage(playerid, COLOR_GRAD2, "   Rozmowa zako�czona...");
						Mobile[playerid] = 1255;
						return 0;
					}
					...
					{
						SendClientMessage(playerid, COLOR_DBLUE, "LSPD HQ: Aktualnie prowadzimy �ledztwo w tej sprawie");
						SendClientMessage(playerid, COLOR_DBLUE, "Dzi�kujemy za zg�oszenie przest�pstwa");
						SendClientMessage(playerid, COLOR_GRAD2, "   Rozmowa zako�czona...");
						Mobile[playerid] = 1255;
						return 0;
					...
					if (badguy == playerid)
					{
						SendClientMessage(playerid, COLOR_DBLUE, "Police HQ: Nie wyg�upiaj si�, to jest linia ratunkowa!");
						SendClientMessage(playerid, COLOR_GRAD2, "   Rozmowa zako�czona...");
						Mobile[playerid] = 1255;
						return 0;
					}
			...
			{
				format(string, sizeof(string), "Police HQ: Nie mamy �adnych informacji o %s, czy to napewno dobre nazwisko?",tmp);
		...
		if(Mobile[playerid] == 912)
		{
			...
			SendClientMessage(playerid, COLOR_DBLUE, "Police HQ: Je�li nie wiesz kto to by�, powiedz poprostu 'nie'.");
			Mobile[playerid] = 913;
			return 0;
		}
