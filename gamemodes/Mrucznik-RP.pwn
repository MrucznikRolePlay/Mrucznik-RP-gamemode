    ...
    if(GetPVarInt(playerid, "patrol") != 0) {
        ...
        if(PatrolInfo[patrol][patroluje][1] != INVALID_PLAYER_ID) {
            ...
            if(PatrolInfo[patrol][patroluje][1] == playerid) {
                ...
                cmd_patrol(playerid, "stop");
                sendTipMessageEx(patrol, COLOR_PAPAYAWHIP, "Partner opuúci≥ patrol. 10-33!");
            } else {
                // nie lider
                cmd_patrol(playerid, "stop");
                cmd_patrol(patrol, "stop");
...
public OnPlayerText(playerid, text[])
{
    ...
    if(GetPVarInt(playerid, "budka-Mobile") != 999) {
        ...
        if(budkaused != 999) {
            format(string, sizeof(string), "%s mÛwi (budka): %s", (PlayerInfo[playerid][pSex] == 1) ? ("MÍøczyzna") : ("Kobieta"), text);
        } else {
