...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(GetPVarInt(playerid, "dutyadmin") == 1)
	{
		...
		//LOG
		getdate(y1, mi1, d1); 
		format(stringlog, sizeof(stringlog), "[%d:%d:%d] Admin %s [%s] zakoÒczy≥ s≥uøbÍ - wykona≥ w czasie %d:%d [B%d/W%d/K%d/I%d] - Wyszed≥ poprzez DISCONNECT", d1, mi1, y1, FirstNickname, AdminName, AdminDutyGodziny[playerid], AdminDutyMinuty[playerid],IloscBan,IloscWarn,IloscKick,IloscInne); //GENERATE LOG
