...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(PlayerInfo[playerid][pInjury] > 0 || PlayerInfo[playerid][pBW] > 0 ) //inna animacja dla bw
	{
		...
		ShowPlayerInfoDialog(playerid, "Mrucznik Role Play", "{FF542E}Jesteú ranny!\n{FFFFFF}Nie moøesz wsiπúÊ do pojazdu.");
    ...
    /*if(killerid != INVALID_PLAYER_ID)
    {
        ...
        if((IsACop(killerid) && OnDuty[killerid] == 1) || FRAC_GROOVE <= frac <= FRAC_VAGOS || frac == FRAC_WPS || frac == FRAC_BOR || frac == 5 || frac == 6 || frac == 8 || frac == 15 || GetPlayerOrgType(killerid) == ORG_TYPE_GANG || GetPlayerOrgType(killerid) == ORG_TYPE_MAFIA)
        {
            ...
            if(inzone || GetSVarInt("BW_OnlyGangZones") == 1)
            {
				...
				SendFamilyMessage(4, COLOR_ALLDEPT, "Centrala: Otrzymano zg≥oszenie: nieprzytomna osoba, chyba oddycha... BojÍ siÍ! Przyjedücie tu!");
...
SetPlayerSpawnPos(playerid)
{
	...
	else if(PlayerInfo[playerid][pInjury] > 0)
	{
		...
		ShowPlayerInfoDialog(playerid, "Mrucznik Role Play", "{FF542E}Jesteú ranny!\n{FFFFFF}Moøesz wezwaÊ pomoc lub poczekaÊ aø ktoú CiÍ dobije."); 
		//dialog
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
   ...
   /* if(newstate == PLAYER_STATE_DRIVER) {
	...
	{
		Player_RemoveFromVeh(playerid);
		return ShowPlayerInfoDialog(playerid, "Mrucznik Role Play", "{FF542E}Jesteú ranny!\n{FFFFFF}Nie moøesz wsiπúÊ do pojazdu.");
	}
	if(newstate == PLAYER_STATE_DRIVER)
    {
        if(newstate == PLAYER_STATE_DRIVER)
