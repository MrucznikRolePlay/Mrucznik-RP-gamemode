...
public OnPlayerDisconnect(playerid, reason)
{
	...
	PlayerInfo[playerid][pLocal] = GetPlayerVirtualWorld(playerid); //local uøyty do przechowania virtual worldu
	...
	if(GetPVarInt(playerid, "dutyadmin") == 1)
	{
		...
		//LOG
		getdate(y1, mi1, d1); 
		format(stringlog, sizeof(stringlog), "[%d:%d:%d] Admin %s [%s] zakoÒczy≥ s≥uøbÍ - wykona≥ w czasie %d:%d [B%d/W%d/K%d/I%d] - Wyszed≥ poprzez DISCONNECT", d1, mi1, y1, FirstNickname, AdminName, AdminDutyGodziny[playerid], AdminDutyMinuty[playerid],iloscBan[playerid],iloscWarn[playerid],iloscKick[playerid],iloscInne[playerid]); //GENERATE LOG
		AdminDutyLog(stringlog); //Create LOG
		
		//Zerowanie zmiennych - po zejúciu z duty admina :) 
