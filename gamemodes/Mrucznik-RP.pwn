...
public OnPlayerConnect(playerid)
{
	...
	Log(connectLog, INFO, "Gracz %s[id: %d, ip: %s, gpci: %s] po��czy� si� z serwerem", GetNick(playerid), playerid, GetIp(playerid), GPCI);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	Log(connectLog, INFO, "Gracz %s[id: %d] roz��czy� si�, pow�d: %d", GetNick(playerid), playerid, reason);
	...
	{
		new string[128];
		format(string, sizeof(string), "[Marcepan Marks] Zabra�em graczowi %s bro� [da� /q podczas AJ DM2]", GetNick(playerid, true));
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if( (IsPlayerConnected(playerid) && gPlayerLogged[playerid] != 1) || (IsPlayerConnected(killerid) && gPlayerLogged[killerid] != 1)) //nie przetwarzaj dla niezalogowanych os�b
	{
		return 1;
	}
		...
		if(PlayerKilledByAdmin[playerid] == 0)
		{
			if(PlayerInfo[playerid][pInjury] > 0)
			{
				if(IsPlayerConnected(killerid) && killerid != INVALID_PLAYER_ID)
				{
					...
					{
						Log(warningLog, INFO, "%s zabi� %s z broni o id %d b�d�c w aucie (mo�liwe DB/CK2).", GetPlayerLogName(killerid), GetPlayerLogName(playerid), reason);
					...
					if(lowcaz[killerid] == playerid && lowcap[playerid] != killerid && poddaje[playerid] != 1)
					{
						...
						SendMessageToAdmin(string, COLOR_YELLOW);
						Log(warningLog, INFO, "�owca nagr�d %s zabi� gracza %s bez oferty /poddajsie", GetPlayerLogName(killerid), GetPlayerLogName(playerid));
					}
					//-------<[    Inne    ]>---------
					if(PlayerPaintballing[playerid] != 0)
					{
...
public OnCheatDetected(playerid, ip_address[], type, code)
{
	...
	if(type == 0) //Type of cheating (when 0 it returns the ID, when 1 - IP)
	{
        ...
        Log(punishmentLog, INFO, "%s dosta� kicka od antycheata, pow�d: kod %d", GetPlayerLogName(playerid), code);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
   ...
   /* if(newstate == PLAYER_STATE_DRIVER) {
	...
	if(gPlayerLogged[playerid] == 0)
	{
		...
		{
			Log(punishmentLog, INFO, "%s dosta� kicka od antycheata, pow�d: spawn b�d�c niezalogowanym");
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has�o
		...
		Log(connectLog, INFO, "Gracz %s zalogowa� si� na konto", GetPlayerLogName(playerid));
	...
	if (PlayerInfo[playerid][pTut] == 1)
	{
        if(PlayerInfo[playerid][pAdmin] > 0 || PlayerInfo[playerid][pNewAP] > 0 || PlayerInfo[playerid][pZG] > 0)
        {
			...
			{
				SendClientMessage(playerid, COLOR_GREEN, "SUPPORT: {FFFFFF}Aby widzie� zg�oszenia z /tickets wpisz {FF0000}/supportduty");
			}
