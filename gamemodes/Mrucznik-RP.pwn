...
Mrucznik� Role Play ----> stworzy� Mrucznik
	Inni developerzy:
		...
		LukeSqly - Niby co� zacz��, ale wysz�o jak zwykle
		Simeone - Kox przez x
		Creative - mo�liwe �e co� potrafi
...
#define RESOURCES_LINK "http://resource_server:8100/"
...
//do implementacji w g��wny kod (mo�liwie w modu�y)
...
public OnGameModeInit()
{
	...
	SetNameTagDrawDistance(70.0); //Wy�wietlanie nick�w od 70 metr�w
...
/*	for(new i = 0; i <= sizeof(Peds)-1; i++)
	...
	levelLock[FRAC_SN][5]=1;//Zamkni�te
    ...
    pusteZgloszenia();
	Log(serverLog, INFO, "Serwer zosta� pomy�lnie uruchomiony.");
			...
			else
			{
				...
				else
				{
					...
					weaponid == 32 || weaponid == 33 || weaponid == 34 || weaponid == 38)
					{
						...
						if(AP > 0)
						{
							if(AP < amount) SetPlayerArmour(hitid, 0); // tutaj ma zosta� 0, bo wartosc armora poni�ej 0 daje 100 armora (samp bug)
						...
						if(HP <= amount) return 1; //wy�lij nab�j (zabij)
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(PlayerInfo[playerid][pJailed] == 2)
	{
		...
		SendClientMessage(playerid, COLOR_GRAD2, "Stra�nik zauwa�y�, �e co� kombinujesz. Wracasz do celi.");
	...
	{
		sendTipMessage(playerid, "Nie mo�esz wej�� do pojazdu podczas biegu!"); 
		GetPlayerPos(playerid, pX,pY,pZ);
		SetPlayerPos(playerid, pX,pY,pZ+2);
	}
	if(GetPlayerPing(playerid) >= 180 && !ispassenger)//Zabezpieczenie, je�li ma za du�y ping
	{
		sendTipMessage(playerid, "Tw�j ping jest stanowczo za wysoki! Odczekaj chwil�, zanim wsi�dziesz do pojazdu"); 
		GetPlayerPos(playerid, pX,pY,pZ);
		SetPlayerPos(playerid, pX,pY,pZ+2);
	}
	...
	// -- customowe parametry dla poszczeg�lnych pojazd�w
	if(IsARower(vehicleid))
	{
		SetVehicleParamsEx(vehicleid, 1, lights, alarm, doors, bonnet, boot, objective);
		engine = 1;
	}
	else if (GetVehicleModel(vehicleid) == 525) sendTipMessageEx(playerid, COLOR_BROWN, "Wsiad�e� do holownika, naci�nij CTRL alby podholowa� w�z.");
...
public OnPlayerConnect(playerid)
{
	...
	Log(connectLog, INFO, "Gracz %s[id: %d, ip: %s, gpci: %s] po��czy� si� z serwerem", GetNickEx(playerid), playerid, GetIp(playerid), GPCI);
	...
	//Nick bez wulgaryzm�w
	if(CheckVulgarityString(nick) == 1)
	{
		SendClientMessage(playerid, COLOR_NEWS, "SERWER: Tw�j nick zawiera wulgaryzmy/niedozwolone s�owa - zmie� go!"); 
		KickEx(playerid);
		return 1;
	}
	SetRPName(playerid);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	Log(connectLog, INFO, "Gracz %s[id: %d] roz��czy� si�, pow�d: %d", GetNickEx(playerid), playerid, reason);
	...
	{
		new string[128];
		format(string, sizeof(string), "[Marcepan Marks] Zabra�em graczowi %s bro� [da� /q podczas AJ DM2]", GetNickEx(playerid));
...
//	if(GetPlayerAdminDutyStatus(playerid) == 1)
	...
	|| firstDutyAdmin[playerid] == 1 && IsAScripter(playerid))//Je�eli admin by� na duty, wykonuje zapis w logi 
	{
		...
		//Log dla 0Verte [Nick][UID] [HH:mm] [Bany] [Warny] [AJ] [Kicki] [Inne] [Reporty+zapytania] [/w] [/w2] [powod zako�czenia s�u�by]
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	Log(damageLog, INFO, "%s zosta� zabity przez %s, pow�d: %d", 
		GetPlayerLogName(playerid),
	...
	if(IsPlayerConnected(playerid))
	{
		...
		if(GetPVarInt(playerid, "skip_bw") == 0)
		{
			if(PlayerInfo[playerid][pInjury] > 0) //TRYB BW (GDY ZGINIE JAK MA RANNEGO)
			{
				...
				if(TalkingLive[playerid] != INVALID_PLAYER_ID)
				{
					SendPlayerMessageToAll(COLOR_NEWS, "NEWS: Wywiad zako�czony - nasz rozm�wca przerwa� wywiad.");
					new talker = TalkingLive[playerid];
					TalkingLive[playerid] = INVALID_PLAYER_ID;
					TalkingLive[talker] = INVALID_PLAYER_ID;
				}
...
public OnPlayerSpawn(playerid)
{
	...
	if(dom != 0)
	{
		...
		{
			format(string, sizeof(string), "* Zosta�e� wykopany z wynajmowanego domu.");
			SendClientMessage(playerid, COLOR_RED, string);
...
SetPlayerSpawnPos(playerid)
{
	...
	else if(PlayerInfo[playerid][pJailed] == 10)//Marcepan Admin Jail
	{
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, string);
        Log(punishmentLog, INFO, "%s da� /q podczas akcji wi�c zabrano mu %d$, %d materia��w oraz bro�.", GetPlayerLogName(playerid), kaseczka, PlayerInfo[playerid][pMats]/2);
    ...
    else
    {
		...
		if(GetPVarInt(playerid, "spawn") == 2)
		{
	        ...
	        {
				sendTipMessage(playerid, "W banku nie wolno mie� broni! Zostanie Ci ona przywr�cona po �mierci.");
				    ...
				    switch(GetPlayerFraction(playerid))
				    {
						...
						case FRAC_SN: //9
						{
							if(PlayerInfo[playerid][pRank] <= 1)
							{
								...
								Wchodzenie(playerid);
								sendTipMessage(playerid, "Zrespi�e� si� na dachu San News"); 
							}
...
SetPlayerSpawnSkin(playerid)
{
	...
	{
		sendTipMessage(playerid, "MRP-SKINS: Wykryto u Ciebie skin eventowy - zostaje Ci ustalona domy�lna warto��");
	...
	if(ScigaSie[playerid] != 666 && IloscCH[playerid] != 0 && Scigamy != 666)
	{
	    ...
	    else if(IloscCH[playerid] == Wyscig[Scigamy][wCheckpointy])
	    {
	        ...
	        if(Ukonczyl == 1)
	        {
		        ...
		        {
		        	format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s wygra� wy�cig %s", GetNickEx(playerid), Wyscig[Scigamy][wNazwa]);
		        	foreach(new i : Player)
		        	{
	        ...
	        else
	        {
	            if(Ukonczyl >= IloscZawodnikow)
		        {
					...
					format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s uko�czy� wy�cig jako ostatni !", GetNickEx(playerid));
					WyscigMessage(COLOR_YELLOW, string);
		            KoniecWyscigu(-2);
		        }
		        else
		        {
					...
					format(string, sizeof(string), "Komunikat wy�cigu: {FFFFFF}%s uko�czy� wy�cig jako %d !", GetNickEx(playerid), Ukonczyl);
					WyscigMessage(COLOR_YELLOW, string);
	            	Ukonczyl++;
		        }
	        }
...
public OnPlayerEditDynamicObject(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	if(IsValidDynamicObject(objectid))
	{
		...
		else if(response == EDIT_RESPONSE_UPDATE && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
        	...
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Musisz by� na swojej strefie!");
                SetDynamicObjectPos(objectid, X, Y, Z);
			}
		}
		else if( response == EDIT_RESPONSE_FINAL && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
			...
			if( GetPVarInt(playerid, "zoneid") == -1 || (ZoneControl[GetPVarInt(playerid, "zoneid")] != frac && ZoneControl[GetPVarInt(playerid, "zoneid")]-100 != GetPlayerOrg(playerid)) )
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Nie by�e� na swojej strefie!");
                graffiti_DeleteMySQL(f);
				graffiti_ZerujZmienne(playerid);
				graffiti_Zeruj(f);
				return 1;
...
public OnRconLoginAttempt(ip[], password[], success)
{
    ...
    else
    {
        if(player != -1)
        {
			...
			{
				new str[128];
				format(str, 128, "RCON: U�ytkownik %s (%d) pr�bowa� si� zalogowa� na rcona bez wymaganych uprawnie�!", GetNickEx(player), player);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)
    {
        	...
        	if(CarData[lcarid][c_OwnerType] == CAR_OWNER_SPECIAL)
        	{
 				if(CarData[lcarid][c_Owner] == RENT_CAR)
    			{
					if (CarData[lcarid][c_Rang]-1 != playerid)
					{
						...
						format(stringDialog, sizeof(stringDialog), "Mo�esz wypo�yczy� ten pojazd\nCena: %d$ za 15 minut", costBike);
		    			TogglePlayerControllable(playerid, 0);
						HireCar[playerid] = vehicleid;
						ShowPlayerDialogEx(playerid, 7079, DIALOG_STYLE_MSGBOX, "Wypo�yczalnia pojazd�w", stringDialog, "Wynajmij", "Wyjd�");
					}
				}
			}
		...
		if(IsACopCar(GetPlayerVehicleID(playerid)) && IsAPolicja(playerid)) sendTipMessageEx(playerid, COLOR_BLUE, "Po��czy�e� si� z komputerem policyjnym, wpisz /mdc aby zobaczy� kartotek� policyjn�");
				...
				else
				{
					if(TransportDuty[i] == 1)
					{
                        ...
                        if(PlayerInfo[playerid][pLevel] < 3)
                        {
                            ...
                            sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Jeste� nowym graczem, obowi�zuje Cie rabat 75 procent na taks�wk�.");
                        }
                        else
                        {
	...
	{// 38 / 49 / 56 = SS
        ...
        if(newcar <= CAR_End) //do kradziezy
        {
		    ...
		    {
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Mo�esz ukra�� ten w�z, wpisz /kradnij aby spr�bowa� to zrobi�.");
				new engine, lights, alarm, doors, bonnet, boot, objective;
        ...
        if(vid != LastVehicleID[playerid])
        {
            if(GetTickDiff(GetTickCount(), VehicleIDChangeTime[playerid]) < 2000)
            {
                ...
                if(VehicleIDChanges[playerid] > MAX_VEHICLE_ID_CHANGES)
                {
                    ...
                    {
                        format(acstr, 128, "%s mo�e lagowa� autami i dosta� kicka U�yj /gotoczit aby to sprawdzi�!", GetNickEx(playerid));
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has�o
		...
		if(PlayerInfo[playerid][pCash] < 0)
		{
			if(PlayerInfo[playerid][pWL] < 9)
			{
				...
				sendTipMessage(playerid, "Masz d�ugi pieni�ne wobec pa�stwa, tw�j poziom poszukiwania ro�nie."); 
			}
			if(PlayerInfo[playerid][pWL] >= 9)
			{
				PlayerInfo[playerid][pWL] = 10; 
				sendTipMessage(playerid, "Masz ju� 10 poziom poszukiwania! Cz�� jest spowodowana d�ugami! Zr�b co� z tym!"); 
			}
		...
		new GPCI[41];
		gpci(playerid, GPCI, sizeof(GPCI));
		Log(connectLog, INFO, "Gracz %s[id: %d, ip: %s, gpci: %s] zalogowa� si� na konto", GetPlayerLogName(playerid), playerid, GetIp(playerid), GPCI);
		...
		{
			format(string, sizeof(string), "Lider�w frakcji w bazie danych: %d.", AllLeaders);
	...
	if (PlayerInfo[playerid][pTut] == 1)
	{
        if(PlayerInfo[playerid][pAdmin] > 0 || PlayerInfo[playerid][pNewAP] > 0 || PlayerInfo[playerid][pZG] > 0 || IsAScripter(playerid))
        {
			...
			{
				SendClientMessage(playerid, COLOR_GREEN, "SUPPORT: {FFFFFF}Aby widzie� zg�oszenia z /tickets wpisz {FF0000}/adminduty");
			}
		...
		if(!IsPlayerInAnyVehicle(playerid))
		{
			...
			if(IsAtPlaceGetHP(playerid))
			{
				if(GetPlayerHealth(playerid, health) <= PlayerInfo[playerid][pHealth])
				{
					...
					{
						sendTipMessage(playerid, "Odczekaj chwil�!"); 
					}
				}
				else
				{
					sendErrorMessage(playerid, "Nie mo�esz wzi�� wi�cej HP!");
					sendTipMessage(playerid, "Aby m�c bra� wi�cej HP ulepsz HP w /ulepszenia!"); 
					return 1;
				}
			}
		}
		...
		if(PRESSED(KEY_FIRE))
		{
			if(GetPlayerWeapon(playerid) == 46)
			{
				...
				if(vehicleid != -1)
				{
					...
					{
						SetPlayerArmedWeapon(playerid, 0); //chowanie spadochronu
        				return ShowPlayerInfoDialog(playerid, "Mrucznik Role Play", "Schowaj spadochron zanim w co� uderzysz."); 
					}
				}
			}
		}
...
public OnPlayerText(playerid, text[])
{
	...
	if(text[0] == '@')
	{
		if(strlen(text) > 31)
		{
			sendTipMessage(playerid, "Nieprawid�owa d�ugo�� znak�w animacji"); 
			return 0;
		}
	...
	if(TalkingLive[playerid] != INVALID_PLAYER_ID)
	{
		...
		{//todo
			if(strlen(text) < 78)
			{
				...
				{
					SendClientMessage(playerid, COLOR_WHITE, "Tw�j wywiad zawiera niedozwolone znaki! (@)");
					return 1;
				}
				format(string, sizeof(string), "%s m�wi: %s", SanNews_nick, text);
		...
		else
		{
			if(strlen(text) < 78)
			{
				...
				format(string, sizeof(string), "%s m�wi: %s", SanNews_nick, text);
				format(wywiad_string, sizeof(wywiad_string), "Go�� wywiadu %s: %s", SanNews_nick, text);
				ProxDetector(10.0, playerid, string, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
				SetPlayerChatBubble(playerid,text,COLOR_FADE1,10.0,8000);
				OOCNews(COLOR_LIGHTGREEN, wywiad_string);
				SendDiscordMessage(DISCORD_SAN_NEWS, wywiad_string);
	...
	if (realchat)
	{
		...
		if(TourettActive[playerid] && GetPlayerAdminDutyStatus(playerid) == 0)
		{
			...
			if(size > 0)
			{
				...
				PlayerTalkIC(playerid, newText, "m�wi", 15.0);
			}
			else
			{
				PlayerTalkIC(playerid, text, "m�wi", 15.0);
			}
		}
