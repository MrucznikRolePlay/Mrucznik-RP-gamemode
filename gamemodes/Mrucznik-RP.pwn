...
public OnPlayerDisconnect(playerid, reason)
{
	...
	{
		format(reString, sizeof(reString), "Poszukiwany %s opuúci≥ San Andreas ((wyszed≥ z gry)).", GetNick(playerid));
	...
	if(IsPlayerConnected(playerid) && playerid != INVALID_PLAYER_ID)
	{
		...
		if(IsPlayerConnected(killerid) && killerid != INVALID_PLAYER_ID && gPlayerLogged[playerid])
		{
			if(!IsACop(killerid) && lowcaz[killerid] != playerid )
			{
				...
				{
					sendTipMessageEx(playerid, COLOR_LIGHTRED, "Masz juø 10 listÛw goÒczych!");
					sendTipMessage(playerid, "Zaczynasz stawaÊ siÍ coraz bardziej smakowity dla ≥owcÛw! Pilnuj siÍ!"); 
				}
			}
		}
	}
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has≥o
		...
		if(PlayerInfo[playerid][pBusinessOwner] != INVALID_BIZ_ID)
		{
			...
			{
				sendErrorMessage(playerid, "Wczytywanie twojego biznesu siÍ nie powiod≥o! Zostaje Ci on odebrany"); 
				sendErrorMessage(playerid, "Jeøeli uwaøasz to za b≥πd skryptu - zg≥oú stratÍ na naszym forum!");
