...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(IsPlayerConnected(playerid) && playerid != INVALID_PLAYER_ID)
	{
		...
		if(IsPlayerConnected(killerid) && killerid != INVALID_PLAYER_ID)
		{
			...
			{
				Log(warningLog, INFO, "Gracz %s zabi≥ %s z broni o id %d bÍdπc w aucie (moøliwe DB/CK2).", GetPlayerLogName(killerid), GetPlayerLogName(playerid), reason);
			...
			if(reason == 41)
			{
				...
				SendMessageToAdmin(string, COLOR_YELLOW);
				Log(warningLog, INFO, "Gracz %s zabi≥ gracza %s uøywajπc spray'a", GetPlayerLogName(killerid), GetPlayerLogName(playerid));
			}
