...
VerifyPlayerIp(playerid)
{
    ...
    if(mysql_num_rows())
    {
        while(mysql_fetch_row_format(query, "|"))
        {
			...
			{
    			mysql_free_result();
				Log(serverLog, INFO, "Nie wymagana zmiana has≥a dla gracza %s (zgodnoúÊ ip %s)", GetNick(playerid), ip);
				return true;
			}
			else
			{
				...
				{
					mysql_free_result();
					Log(serverLog, INFO, "Nie wymagana zmiana has≥a dla gracza %s (zgodnoúÊ hosta ip %s)", GetNick(playerid), ip);
					return true;
				}
			}
        }
...
PasswordConversion(playerid, password[])
{
	if(strlen(password) == 32)
	{
		...
		if(strcmp(password, MD5_Hash(password), true ) == 0)
		{
			...
			Log(serverLog, WARNING, "Konwersja hasla konta %s z MD5 na hash whirlpool + sÛl", GetNick(playerid));
	...
	else
	{
		...
		if(strcmp(password, hashedPassword, true ) == 0)
		{
			//convert
			MruMySQL_ChangePassword(GetNick(playerid), password);
			Log(serverLog, WARNING, "Konwersja hasla konta %s z whirlpool na hash whirlpool + sÛl", GetNick(playerid));
