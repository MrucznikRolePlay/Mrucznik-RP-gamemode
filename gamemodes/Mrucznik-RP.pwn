	...
	if(checkpointid == navigationCheckPoint[playerid])
	{
		if(PlayerInfo[playerid][pJob] == JOB_TRUCKER)
		{
			if(pZlecenieID[playerid] != INVALID_BUSINESSID)
			{
				...
				{
					sendTipMessageEx(playerid, COLOR_GREEN, "Gratulacje dotar≥eú do celu!"); 
					format(string, sizeof(string), "%s mÛwi: DziÍkujÍ za tak szybkπ dostawÍ. ProszÍ oto paÒskie wynagrodzenie {BA55D3}**podaje  %d gotÛwki**{FFFFFF}.", GetBusinessOwnerName(bIDE), pZlecenieCost[playerid]);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(IsPlayerConnected(playerid) && playerid != INVALID_PLAYER_ID)
	{
		...
		{
			format(string, sizeof(string), "Zginπ≥eú podczas przewozu $%d gotÛwki - tracisz jπ.", PlayerMoneyFromBiz[playerid]); 
			sendTipMessage(playerid, string); 
			DetachPlayerItem(playerid, GetIndexFromAttachedObjectModel(playerid, 19624)); 
			if(IsAPrzestepca(killerid))
			{
				format(string, sizeof(string), "Zabi≥eú %s gdy ten przewozi≥ $%d gotÛwki z swojego biznesu! Otrzymujesz $%d",
				GetNick(playerid),
				...
				format(string, sizeof(string), "%s zabi≥ %s gdy ten przewozi≥ $%d gotÛwki - otrzyma≥ $%d", 
				GetNick(killerid), 
				GetNick(playerid),
				PlayerMoneyFromBiz[playerid],
				(PlayerMoneyFromBiz[playerid]/4));
...
public OnPlayerEditDynamicObject(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
    if(IsValidDynamicObject(objectid))
	{
		...
		else if(response == EDIT_RESPONSE_UPDATE && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
        	...
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Musisz byÊ na strefie!");
                SetDynamicObjectPos(objectid, X, Y, Z);
			}
		}
		else if( response == EDIT_RESPONSE_FINAL && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
			...
			if(GetPVarInt(playerid, "zoneid") == -1) 
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Nie by≥eú na strefie!");
                graffiti_DeleteMySQL(f);
				graffiti_ZerujZmienne(playerid);
				graffiti_Zeruj(f);
				return 1;
