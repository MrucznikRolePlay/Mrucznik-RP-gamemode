...
#include <YSI\YSI\y_dialog> */ // Øegnaj YSI dobry druchu :( ale to nie na te lata...
...
public OnPlayerCommandReceived(playerid, cmdtext[])
{
	...
	{
		_MruGracz(playerid, "SERWER: "SZARY"Nie jesteú zalogowany/Masz otwarte okno dialogowe!");
		return 0;
	}
    if(GetTickCount() - StaryCzas[playerid] < 100)//antyspam
	{
		_MruGracz(playerid, "SERWER: "SZARY"Odczekaj chwilÍ zanim wpiszesz nastÍpnπ komende!");
		return 0;
	}
    if(IsCommandBlocked(cmdtext))
    {
        _MruGracz(playerid, "SERWER: "SZARY"Komenda jest wy≥πczona.");
        return 0;
    }
...
SetPlayerSpawnPos(playerid)
{
	...
	else if(PlayerInfo[playerid][pJailed] == 3)
	{
		...
		_MruGracz(playerid, "Gra≥eú NON-RP. Wracasz do Admin Jaila.");
	}
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(PlayerInfo[playerid][pLinia72]==1)
	{
		...
		else if(CP[playerid]==729)
		{
			...
			_MruGracz(playerid, "|_____________Komunikat_____________|");
			_MruGracz(playerid, "Gratulacje! UkoÒczy≥eú trasÍ do Mrucznik Tower! (1/2 zadania). Moøesz tu odpoczπÊ.");
    ...
    if(PlayerInfo[playerid][pLinia96]==1)
	{
		...
		else if(CP[playerid]==972)
		{
			...
			_MruGracz(playerid, "|_____________Komunikat_____________|");
			_MruGracz(playerid, "Gratulacje! UkoÒczy≥eú trasÍ do Mrucznik Tower! (1/2 zadania). Moøesz odpoczπÊ.");
			_MruGracz(playerid, "Jeøeli chcesz jechaÊ dalej wykonaj nawrÛt kierujπc siÍ do kolejnego checkpointu!");
			_MruGracz(playerid, "|_____________>>LSBD GPS<<_____________|");
			PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
			_MruGracz(playerid, "+310$");
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has≥o
		...
		if(PlayerInfo[playerid][pBlock] == 1)
		{
			...
			_MruGracz(playerid, "[SERVER] Jeúli uwaøasz, øe konto zosta≥o zablokowane nies≥usznie napisz apelacje na: {33CCFF}www.Mrucznik-RP.pl");
			KickEx(playerid);
			return 1;
		}
	...
	/*if(dini_Exists("Filtry.ini"))
	{
		...
		if(filtr != 0)
		{
			...
			_MruGracz(playerid,"SERVER: Aby go zdjπÊ napisz na forum www.Mrucznik-RP.pl");
			KickEx(playerid);
			return 1;
		}
	}
	...
	if (PlayerInfo[playerid][pTut] == 1)
	{
        if(PlayerInfo[playerid][pAdmin] > 0 || PlayerInfo[playerid][pNewAP] > 0 || PlayerInfo[playerid][pZG] > 0)
        {
            if(PlayerInfo[playerid][pZG] > 0 || PlayerInfo[playerid][pNewAP] > 0)
            {
                ...
                _MruGracz(playerid, "SUPPORT: {FFFFFF}Stawiasz siÍ na s≥uøbie nowym graczom. Aby sprawdziÊ zg≥oszenia wpisz {00FF00}/tickets");
            }
	...
	if(ConnectedToPC[playerid] == 255)
	{
		...
		else if ((strcmp("Order", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("Order")))
		{
			...
			else
			{
			    ...
			    if(PlayerInfo[playerid][pRank] >= 0) { _MruGracz(playerid, "|(1) ($2500) Ranga 0: NÛø, Desert Eagle, Shotgun, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 1) { _MruGracz(playerid, "|(2) ($5000) Ranga 1: NÛø, Desert Eagle, MP5, Shotgun, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 2) { _MruGracz(playerid, "|(3) ($6000) Ranga 2: NÛø, Desert Eagle, M4, MP5, Shotgun, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 2) { _MruGracz(playerid, "|(4) ($6000) Ranga 2: NÛø, Desert Eagle, AK47, MP5, Shotgun, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 3) { _MruGracz(playerid, "|(5) ($8000) Ranga 3: NÛø, Desert Eagle, M4, MP5, Shotgun, Snajperka, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 3) { _MruGracz(playerid, "|(6) ($8000) Ranga 3: NÛø, Desert Eagle, AK47, MP5, Shotgun, Snajperka, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 4) { _MruGracz(playerid, "|(7) ($8500) Ranga 4: NÛø, Desert Eagle, M4, MP5, Spas12, Snajperka, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 4) { _MruGracz(playerid, "|(8) ($8500) Ranga 4: NÛø, Desert Eagle, AK47, MP5, Spas12, Snajperka, Pancerz"); }
			    if(PlayerInfo[playerid][pRank] >= 5) { _MruGracz(playerid, "|(9) ($10000) Ranga 5-9: NÛø, Desert Eagle, M4, UZI, Spas12, Snajperka, Pancerz"); }
                if(PlayerInfo[playerid][pRank] >= 5) { _MruGracz(playerid, "|(10) ($10000) Ranga 5-9: NÛø, Desert Eagle, AK47, UZI, Spas12, Snajperka, Pancerz"); }
				_MruGracz(playerid, "|________________________________________________________|");
			    return 0;
			}
		}
		...
		{
		    _MruGracz(playerid, "|___ Hitman Agency ___|");
		    _MruGracz(playerid, "| - News");
		    _MruGracz(playerid, "| - Kontrakty");
		    _MruGracz(playerid, "| - Givehit");
		    _MruGracz(playerid, "| - Backup");
		    _MruGracz(playerid, "| - Order");
		    _MruGracz(playerid, "| - Rangi");
		    _MruGracz(playerid, "| - Wyloguj");
		    _MruGracz(playerid, "|");
			_MruGracz(playerid, "|______________|00:00|");
		    return 0;
		}
	    return 0;
	}
