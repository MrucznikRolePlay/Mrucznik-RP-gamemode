...
public OnPlayerDeath(playerid, killerid, reason)
{
       ...
       // if((IsACop(killerid) && OnDuty[killerid] == 1) || FRAC_GROOVE <= frac <= FRAC_VAGOS || frac == FRAC_WPS || frac == FRAC_BOR || frac == 5 || frac == 6 || frac == 8 || frac == 15 || GetPlayerOrgType(killerid) == ORG_TYPE_GANG || GetPlayerOrgType(killerid) == ORG_TYPE_MAFIA)
        //{
            ...
            if(inzone || GetSVarInt("BW_OnlyGangZones") == 1)
            {
				...
				SendFamilyMessage(4, COLOR_ALLDEPT, "Centrala: Do wszystkich jednostek! Nadawca: åwiadek");
				SendFamilyMessage(4, COLOR_ALLDEPT, "Centrala: Otrzymano zg≥oszenie: nieprzytomna osoba, chyba oddycha... BojÍ siÍ! Przyjeüdzie tu!");
	...
	if(IsPlayerConnected(playerid) && playerid != INVALID_PLAYER_ID)
	{
		...
		if(TalkingLive[playerid] != INVALID_PLAYER_ID)
		{
			SendPlayerMessageToAll(COLOR_NEWS, "NEWS: Wywiad zakoÒczony - nasz rozmÛwca jest nieprzytomny.");
			new talker = TalkingLive[playerid];
			TalkingLive[playerid] = INVALID_PLAYER_ID;
			TalkingLive[talker] = INVALID_PLAYER_ID;
		}
		...
		if(Mobile[playerid] != INVALID_PLAYER_ID)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "Jesteú nieprzytomny - po≥πczenie zakoÒczone.");
			if(Mobile[playerid] >= 0)
			{
