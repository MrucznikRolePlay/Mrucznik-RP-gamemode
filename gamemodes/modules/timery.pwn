	...
	foreach(Player, i)
	{
		...
		if(GetPlayerPing(i) >= 2000 && PlayerInfo[i][pAdmin] == 0)
		{
			if(gPlayerLogged[i] == 1)
			{
				...
				{
					SendClientMessage(i, COLOR_PANICRED, "Dosta³e kicka, POWÓD: Ping wiêkszy ni¿ 2 000");
					KickEx(i);
				}
			}
		}
                    ...
                    switch(weaponID)
                    {
                        ...
                        case 4://nó¿
    					{
    						if(PlayerInfo[i][pGun1] != 4 && PlayerInfo[i][pMember] != 8 && PlayerInfo[i][pLider] != 8)
    						{
                                ...
                                {
        							SendClientMessage(i, COLOR_PANICRED, "Dosta³e kicka od systemu, powód: Spawn Broni");
        							KickEx(i);
                                }
    						}
    					}
			...
			if(gPlayerLogged[i] == 1 || TutTime[i] >= 1)
			{
				if(playerState == 1 || playerState == 2 || playerState == 3)
				{
                    ...
                    switch(weaponID)
                    {
    					...
    					case 42:
    					{
    						format(string, sizeof(string), "* %s wyci¹ga Ganicê.", specNAME);
    						ProxDetector(10.0, i, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
    					}
			...
			if(OrderReady[i] > 0)
			{
				switch (OrderReady[i])
				{
				    case 1:
					{
						...
						_MruAdmin(i, "Odebra³e swoje zamówienie");
					}
					case 2:
					{
    ...
    foreach(Player, i)
	{
		...
		if(PlayerInfo[i][pJailed] > 0)
		{
			...
			if(PlayerInfo[i][pJailTime] <= 0 && WantLawyer[i] == 0)
			{
				...
				PlayerInfo[i][pJailed] = 0;
				SendClientMessage(i, COLOR_GRAD1,"   Wolnoæ! Odsiedzia³e karê, mamy nadzieje ¿e to ciê czego nauczy³o.");
		...
		if(State == PLAYER_STATE_DRIVER)
		{
			if(IsPlayerInRangeOfPoint(i, 7.0, 2064.0703,-1831.3167,13.3853) || IsPlayerInRangeOfPoint(i, 7.0, 1024.8514,-1022.2302,31.9395) || IsPlayerInRangeOfPoint(i, 7.0, 486.9398,-1742.4130,10.9594) || IsPlayerInRangeOfPoint(i, 7.0, -1904.2325,285.3743,40.8843)  || IsPlayerInRangeOfPoint(i, 7.0, 720.0876,-458.3574,16.3359))
			{
				if(naprawiony[i] == 0)
				{
					...
					if(health <= 999)
					{
				        sendTipMessageFormat(i, "Zap³aci³e $%d za wizytê w warsztacie", 2500);
				        ZabierzKase(i, 2500);
        ...
        if(GetPVarInt(i, "budka-used") != 999) {
            ...
            if(GetPlayerDistanceFromPoint(i, budki[budkaid][b_x], budki[budkaid][b_y], budki[budkaid][b_z]) > 3.5) {
                sendTipMessage(i, "Oddali³e siê zbytnio od budki, po³¹czenie przerwane...", COLOR_PAPAYAWHIP);
		...
		if(TutTime[i] >= 1 && !IsPlayerNPC(i))
		{
			...
			if(TutTime[i] == 3)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Ooo... nowy na serwerze.... wiêc musisz o czym wiedzieæ.");
			...
			else if(TutTime[i] == 16)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Nie chcemy na serwerze osób które bezmylnie zabijaj¹ wszystko co siê rusza.");
				SendClientMessage(i, COLOR_WHITE, "Chodzi o to, ¿e w prawdziwym ¿yciu, nie zabija³by wszystkich dooko³a.");
				SendClientMessage(i, COLOR_WHITE, "Wiêc jeli chcesz kogo zabiæ, musisz mieæ naprawdê wa¿ny powód.");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze trzeba mieæ nick typu Imie_Nazwisko (np. Jan_Kowalski)");
				SendClientMessage(i, COLOR_WHITE, "Jeli masz inny nick ni¿ Imie_Nazwisko to popro admina o zmianê go.");
			}
			...
			{
				SendClientMessage(i, COLOR_WHITE, "Je¿eli widzisz, ¿e kto cheatuje, powiadom administratorów przez komendê /report.");
				SendClientMessage(i, COLOR_WHITE, "Nie wolno u¿ywaæ Bugów (np. znasz jakiego buga który daje ci kase).");
				SendClientMessage(i, COLOR_WHITE, "Jeli masz czity, wy³¹cz je i id na jaki inny serwer. Tu nie mo¿na czitowaæ.");
			...
			else if(TutTime[i] == 76)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "IC to tak jakby przeciwnoæ OOC. To wszystko co JEST zwi¹zane z twoj¹ postaci¹.");
			...
			else if(TutTime[i] == 98)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Lub wtedy gdy kto mówi na /b jestem liderem LCN, a ty siê go pytasz o pracê w LCN");
			...
			else if(TutTime[i] == 114)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "zapewne masz jeszcze sporo pytañ dotycz¹cych gry, znajdziesz na nie odpowied");
				...
				if(trigger)
				{
					if(Lost == 1)
					{
						if(IsPlayerConnected(Boxer1) && IsPlayerConnected(Boxer2))
						{
							...
							if(TBoxer < 255)
							{
								if(IsPlayerConnected(TBoxer))
								{
									if(TBoxer != Boxer2)
									{
										...
										{
											_MruGracz(Boxer2, "* Zosta³by mistrzem bokserskim gdyby mia³ pracê boksera!");
										}
									}
									else
									{
							...
							format(string, sizeof(string), "* Wygra³e walkê z %s.", loser);
							_MruGracz(Boxer2, string);
							GameTextForPlayer(Boxer2, "~r~Wygrales", 3500, 1);
							if(GetPlayerHealth(Boxer1, health) < 20)
							{
								_MruGracz(Boxer1, "* Czujesz siê wyczerpany, id co zjeæ.");
							...
							{
								_MruGracz(Boxer2, "* Czujesz siê wykoñczony, id co zjeæ.");
					...
					else if(Lost == 2)
					{
						if(IsPlayerConnected(Boxer1) && IsPlayerConnected(Boxer2))
						{
							...
							if(TBoxer < 255)
							{
								if(IsPlayerConnected(TBoxer))
								{
									if(TBoxer != Boxer1)
									{
										...
										{
											_MruGracz(Boxer1, "* Zosta³by mistrzem bokserskim gdyby mia³ pracê boksera !");
										}
									}
									else
									{
							...
							{
								_MruGracz(Boxer2, "* Czujesz siê wyczerpany, id co zjeæ.");
								SetPlayerHealth(Boxer2, 30.0);
							}
							else
							{
		...
		if(StartingKartRound == 1 && AnnouncedKartRound == 0)
		{
			...
			{
				SendClientMessage(i, COLOR_YELLOW, "Wycig gokartów rozpocznie siê za 15 sekund (Aby do³¹czy³o wiêcej uczestników).");
			}
		}
	...
	foreach(Player, i)
   	{
		...
		if(Refueling[i] == 1)
		{
			...
			{
				format(string,sizeof(string),"Nie posiadasz doæ pieniêdzy ($%d) aby zatankowaæ ten pojazd.",FillUp);
...
public CarCheck()
{
	...
	foreach(Player, j)
	{
        if(kaska[j] < 0)
		{
			if(MoneyMessage[j]==0)
			{
				format(string, sizeof(string), "Masz d³ugi, musisz zarobiæ do nastêpnej wyp³aty $%d inaczej nalemy na ciebie policje.",PlayerInfo[j][pCash]);
