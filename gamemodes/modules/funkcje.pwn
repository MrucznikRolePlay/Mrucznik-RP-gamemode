...
stock HABox(numer, playerid)
{
    ...
    {
        _MruGracz(playerid, "Kontrakt który podpisa³e zosta³ odebrany przez Hitmanów. Spodziewaj siê wykonania zlecenia.");
    }
	return 1;
}
...
KoniecWyscigu(playerid)
{
	...
	if(playerid == -1)
		...
		format(string, sizeof(string), "Komunikat wycigowy: {FFFFFF}Wyscig %s zakoñczony przez %s.", Wyscig[Scigamy][wNazwa], GetNick(playerid, true));
		...
		if(IsPlayerConnected(i))
		{
		    if(PlayerInfo[i][pLottoNr] > 0)
		    {
			    if(PlayerInfo[i][pLottoNr] == number)
			    {
					...
					format(string, sizeof(string), "Totolotek: %s Wygra³ nagrodê w wysokoci: $%d.", winner, Jackpot);
					OOCOff(COLOR_WHITE, string);
					PayLog(string);
					format(string, sizeof(string), "* Wygra³e nagrodê o wysokoci: $%d dziêki wytypowaniu prawid³owej liczby !", Jackpot);
					SendClientMessage(i, COLOR_YELLOW, string);
			    	DajKase(i, Jackpot);
			    }
			    else
			    {
			        SendClientMessage(i, COLOR_LIGHTBLUE, "* Tym razem nie wygra³e w totolotku. Spróbuj innym razem :) .");
			    }
			}
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "Zabiæ:[%d] mierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Rodzina:[%s]",kills,deaths,costlevel,exp,expamount,wanted,f2text);
...
Niwelacje(playerid)
{
	...
	{
		format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d MATSÓW DO 0$. JE¯ELI UWA¯ASZ, ¯E NIES£USZNIE - ZG£O STRATÊ NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pMats],PlayerInfo[playerid][pLevel]);
	...
	if(PlayerInfo[playerid][pLevel] == 1)
	{
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JE¯ELI UWA¯ASZ, ¯E NIES£USZNIE - ZG£O STRATÊ NA FORUM. AKUTALNY LVL: %d",kaska[playerid],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
			strins(kox, playername2, 0);
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JE¯ELI UWA¯ASZ, ¯E NIES£USZNIE - ZG£O STRATÊ NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pCash],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JE¯ELI UWA¯ASZ, ¯E NIES£USZNIE - ZG£O STRATÊ NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pAccount],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
			strins(kox, playername2, 0);
...
stock Oil_Destroy(lID)
{
    ...
    foreach(Player, i)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[LSFD] Usun¹³e plamê oleju! Otrzymujesz 2 500$! [LSFD]");
        DajKase(i, 2500);
