...
public OznaczCzitera(playerid)
{
	...
	format(string, sizeof(string), "UWAGA! %s[%d] posiada s0beita na 99\%. Zbanuj go, gdy uøyje czitÛw, bπdü po pewnym czasie gry, aby nie wy≥πczy≥ czitÛw przy wejúciu na serwer!", GetNick(playerid), playerid);
	SendAdminMessage(COLOR_PANICRED, string);
	SendAdminMessage(COLOR_PANICRED, "Gracz ten zosta≥ dodany do listy cziterÛw, wpisz /cziterzy aby zobaczyÊ tπ listÍ.");
	return 1;
}
...
stock Taxi_FareEnd(playerid)
{
	...
	format(string, sizeof(string), "* ZakoÒczy≥eú s≥uøbÍ, zarobi≥eú: $%d.", TransportMoney[playerid]);
	SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
	if(GetPlayerFraction(playerid) != FRAC_KT) DajKase(playerid, TransportMoney[playerid]);
    else
    {
        Sejf_Add(FRAC_KT, TransportMoney[playerid]/2);
...
function sprawdzwejdz(playerid, Float:XsprawdzX, Float:YsprawdzY, Float:ZsprawdzZ)
{
	...
	if(plPosZ+5 < ZsprawdzZ)
	{
		...
		SendClientMessage(playerid, COLOR_LIGHTRED, "Zosta≥eú teleportowany do drzwi, gdyø wykryto spadanie twojej postaci.");
	}
	return 1;
}
...
KoniecWyscigu(playerid)
{
	...
	if(playerid == -1)
		...
		format(string, sizeof(string), "Komunikat wyúcigowy: {FFFFFF}Wyscig %s zakoÒczony przez %s.", Wyscig[Scigamy][wNazwa], GetNick(playerid));
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "ZabiÊ:[%d] åmierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Rodzina:[%s]",kills,deaths,costlevel,exp,expamount,wanted,f2text);
...
public MRP_ShopPurchaseCar(playerid, model, cena)
{
    ...
    if(carid >= MAX_CARS)
    {
        SendClientMessage(playerid, COLOR_PANICRED, "Nie moøna stworzyÊ pojazdu! Moøliwe przepe≥nienie, auto zosta≥o kupione lecz nie moøesz go uøyÊ.");
        return 1;
    }
...
{
    new str[256], model, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, Float:sx, Float:sy, Float:sz, uid, active;
    format(str, 256, "SELECT `model`, `id`, `x`, `y`, `z`, `rx`, `ry`, `rz`, `sx`, `sy`, `sz`, `active` FROM `mru_playeritems` WHERE `UID`='%d' ORDER BY `order`", PlayerInfo[playerid][pUID]);
    mysql_query(str);
    mysql_store_result();
    while(mysql_fetch_row_format(str, "|"))
    {
        sscanf(str, "p<|>ddfffffffffd", model, uid, x, y, z, rx, ry, rz, sx, sy, sz, active);
