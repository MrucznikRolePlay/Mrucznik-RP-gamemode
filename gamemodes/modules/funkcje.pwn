...
{
	new string[32];
	SendClientMessage(playerid, COLOR_RED, "Lista cziterÛw u ktÛrych wykryto s0beita:");
	foreach(Player, i)
	{
...
stock Taxi_FareEnd(playerid)
{
	...
	format(string, sizeof(string), "ZakoÒczy≥eú s≥uøbÍ, zarobi≥eú $%d dla Korporacji Transportowej", TransportMoney[playerid]);
...
stock DopalaczPD(playerid)
{
	...
	if(Velocity[2] > 0.95)
	{
		SendClientMessage(playerid, TEAM_CYAN_COLOR, "Aktywowa≥eú dopalacz!!!");
...
stock HABox(numer, playerid)
{
    ...
    {
        SendClientMessage(playerid, COLOR_LIGHTBLUE, "Kontrakt ktÛry podpisa≥eú zosta≥ odebrany przez HitmanÛw. Spodziewaj siÍ wykonania zlecenia.");
    }
	return 1;
}
	...
	foreach(Player, i)
	{
	    if(IsPlayerConnected(i))
	    {
	        if(PlayerPaintballing[i] != 0)
	        {
	            ...
	            SendClientMessage(i, COLOR_YELLOW, "Mecz Pintballa rozpoczÍty, skoÒczy siÍ za 4 minuty.");
...
public PreparePaintball()
{
    foreach(Player, i)
	{
		if(PlayerPaintballing[i] != 0)
		{
			SendClientMessage(i, COLOR_YELLOW, "Mecz Pintballa rozpocznie siÍ za 20 sekund, proszÍ czekaÊ.");
		}
	}
...
public zabezpieczeniewoz(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
	    SendClientMessage(playerid, COLOR_PANICRED, "Dosta≥eú kicka za prawdopodobieÒstwo uøycia czitÛw. Jeúli to bug, napisz na forum: www.mrucznik-rp.net");
		KickEx(playerid);
	}
	return 1;
}*/
...
/*public UzyteKajdany(playerid,giveplayerid)
{
	if(PDkuje[playerid] > 0 && PlayerInfo[giveplayerid][pJob] == 1)
	{
		...
		format(string, sizeof(string), "* %s nie stawia oporu i daje siÍ skuÊ %s.", GetNick(playerid), GetNick(giveplayerid));
		ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
 		format(string, sizeof(string), "Sku≥eú %s. Masz 2 minuty, by dostarczyÊ go do celi!", GetNick(playerid));
...
public Next(playerid, msg[])
{
    ...
    ProxDetector(15.0, playerid, msg, COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);//system autobusÛw
    return 1;
...
KoniecWyscigu(playerid)
{
	...
	if(playerid == -1)
		...
		format(string, sizeof(string), "Komunikat wyúcigowy: {FFFFFF}Wyscig %s zakoÒczony przez %s.", Wyscig[Scigamy][wNazwa], GetNick(playerid, true));
...
public TablicaWynikow(playerid)
{
	foreach(Player, i)
	{
		...
		{
			SendClientMessage(i, COLOR_LIGHTGREEN, "Tabela wynikÛw:");
            foreach(Player, di)
			{
			    if(zawodnik[di] == 1)
			    {
			        if(okregi[di] >= 1)
			        {
				        ...
				        format(string, sizeof(string), "%s - %d okrπøeÒ", iplayer, okregi[di]);
...
public PaintballEnded()//nowe domy biznes waøne
{
	new string[256];
...
/*IsBusStop(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		{//Trzecie 4 (Motel(nie ma), Molo?, Zwiπzek EmerytÛw, Salon Samochodowy)
		    return 1;
		}
		...
		{//Piπte 4 (Kasyno Liderowe, Cywilarnia, Wyporzyczalnia wozÛw, Salon SamochodÛw Terenowych + ten motel co nie by≥o w trzeciej 4 zmieniony na bazÍ wojskowπ)
		    return 1;
		}
	}
	return 0;
...
PrzywrocBron(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		...
		if(PlayerInfo[playerid][pGun8] >= 2 && PlayerInfo[playerid][pAmmo8] >= 10)
		{
...
WejdzInt(playerid, Float:x, Float:y, Float:z, Float:x2, Float:y2, Float:z2, Float:tolerancja, interior, vw, komunikat[]="", local, gametext[]="")
{
    if (IsPlayerInRangeOfPoint(playerid, tolerancja, x, y, z))
    {
        ...
        {
            if(PlayerInfo[playerid][pLevel] < 3) return sendTipMessageEx(playerid, COLOR_GRAD1, "Tylko gracze z conajmniej 3 lvl mogπ graÊ w kasynie!");
			
            SendClientMessage(playerid, COLOR_GREEN, "Witamy w Bymber Casino.");
            SendClientMessage(playerid, COLOR_WHITE, "W naszym kasynie obowiπzujπ nastÍpujπce stawki za rozpoczÍcie gry:");
            SendClientMessage(playerid, COLOR_GREEN, "Kostki - (0.5 proc. podatku) za rzut /kostka || Black Jack - 100$ za kartÍ /oczko");
            SendClientMessage(playerid, COLOR_GREEN, "Ko≥o fortuny - 5 000$ za obrÛt /kf || Ruletka - 10 000$ za zakrÍcenie /ruletka");
        }
		
		if(vw == 55)
		{
            SendClientMessage(playerid, COLOR_PANICRED, "****Piip! Piip! Piip!*****");
            SendClientMessage(playerid, COLOR_WHITE, "Przechodzπc przez wykrywacz metalu s≥yszysz alarm.");
            SendClientMessage(playerid, COLOR_WHITE, "Okazuje siÍ, øe do kasyna nie mozna wnosiÊ broni.");
            SendClientMessage(playerid, COLOR_WHITE, "Nie chcesz k≥opotÛw, wiÍc oddajesz swÛj arsena≥ ochronie.");
            SendClientMessage(playerid, COLOR_PANICRED, "((broÒ zostanie przywrÛcona po úmierci lub ponownym zalogowaniu))");
		...
		if(local == PLOCAL_INNE_BANK && x == 1462.395751)//DLA BANKU LS
		{
			...
			sendTipMessage(playerid, "* Aby zarzπdzaÊ swoim kontem wpisz /kontobankowe (/kb)");
			sendTipMessage(playerid, "* Aby zarzπdzaÊ kontem swojej frakcji przejdü w zak≥adkÍ ''Frakcyjne''");
			sendTipMessage(playerid, "* Sejf znajduje siÍ 10m pod ziemiπ --> Bezpieczna lokata!");
		...
		if(local == PLOCAL_INNE_BANK && x == 2302.7798)//DLA BANKU W PC
		{
			...
			sendTipMessage(playerid, "* Aby zarzπdzaÊ swoim kontem wpisz /kontobankowe (/kb)");
			sendTipMessage(playerid, "* Aby zarzπdzaÊ kontem swojej frakcji przejdü w zak≥adkÍ ''Frakcyjne''");
			sendTipMessage(playerid, "* Sejf znajduje siÍ  6m pod ziemiπ --> Bezpieczna lokata!");
		}
		if(local == PLOCAL_FRAC_FBI && PlayerInfo[playerid][pMember] >=3)
		{	
			if(doorFBIStatus == 0)
			{
				SendClientMessage(playerid, COLOR_WHITE, "Drzwi sπ zamkniÍte"); 
				return 1;
			}
			if(doorFBIStatus == 1)
			{
				SendClientMessage(playerid, COLOR_LIGHTGREEN, ">>>> Biurowiec FBI w Los Santos Wita! <<<<");
				SendClientMessage(playerid, COLOR_WHITE, "-> Recepcja znajduje siÍ po twojej lewej stronie");
				SendClientMessage(playerid, COLOR_WHITE, "-> Wejúcie do wiÍzienia stanowego na wprost"); 
				SendClientMessage(playerid, COLOR_WHITE, "-> Winda znajduje siÍ za recepcjπ");
		...
		if(local == PLOCAL_FRAC_DMV && x == 1479.9545)
		{
			if(dmv == 1 || IsAnInstructor(playerid) || IsABOR(playerid))
            {
                ...
                {
                    SendClientMessage(playerid, COLOR_LIGHTGREEN, ">>>> Urzπd Miasta w Los Santos Wita! <<<<");
                    SendClientMessage(playerid, COLOR_WHITE, "-> Cennik znajduje siÍ zaraz za rogiem, po prawej stronie.");
                    SendClientMessage(playerid, COLOR_WHITE, "-> Znajdujesz siÍ na najwyøszym poziomie, winda znajduje siÍ w holu g≥Ûwnym");
                    SendClientMessage(playerid, COLOR_WHITE, "-> Okienka dla patentÛw znajdujπ siÍ po lewej i prawej stronie w holu pierwszym");
					SendClientMessage(playerid, COLOR_WHITE, "-> [Obecny interior urzÍdu powsta≥ w listopadzie 2018 roku, za inicjatywπ Satius & Arkam & Simeone]");
                    SendClientMessage(playerid, COLOR_LIGHTGREEN, ">>>> Øyczymy przyjemnego czekania na licencje! <<<<");
					...
					&& GetPlayerOrg(playerid) == 0)//Nie jest cz≥onkiem ORG
                    {
                        SendClientMessage(playerid, COLOR_PANICRED, "****Piip! Piip! Piip!*****");
                        SendClientMessage(playerid, COLOR_WHITE, "Przechodzπc przez wykrywacz metalu s≥yszysz alarm.");
                        SendClientMessage(playerid, COLOR_WHITE, "Dopiero teraz dostrzegasz czerwonπ tabliczkÍ informujπcπ o zakazie");
                        SendClientMessage(playerid, COLOR_WHITE, "Nie chcesz k≥opotÛw, wiÍc oddajesz swÛj arsena≥ agentowi USSS.");
                        SendClientMessage(playerid, COLOR_PANICRED, "((BroÒ otrzymasz po úmierci//ponownym zalogowaniu))");
                        SetPVarInt(playerid, "mozeUsunacBronie", 1);
                        ResetPlayerWeapons(playerid);
                    }
                }
                ...
                {
                    SendClientMessage(playerid, COLOR_RED, "Zosta≥eú wyrzucony z UrzÍdu przez agentÛw USSS, sprÛbuj pÛüniej.");
            ...
            else
            {
				...
				SendClientMessage(playerid,COLOR_RED,"|_________________Godziny pracy UrzÍdu_________________|");
				SendClientMessage(playerid,COLOR_WHITE,"                   {ADFF2F}ßPoniedzia≥ek - Piπtek:");
				SendClientMessage(playerid,COLOR_WHITE,"                          Od 18:00 do 19:00");
				SendClientMessage(playerid,COLOR_WHITE,"");
				SendClientMessage(playerid,COLOR_RED,"             **********************************************");
				SendClientMessage(playerid,COLOR_WHITE,"                  {DDA0DD}ßSobota- Niedziela");
				SendClientMessage(playerid,COLOR_WHITE,"                          Od 15:00 do 16:00");
				SendClientMessage(playerid,COLOR_WHITE,"");
				SendClientMessage(playerid,COLOR_RED,"|____________>>> Urzπd Miasta Los Santos <<<____________|");
				return 1;
            }
...
Lotto(number)
{
		...
		if(IsPlayerConnected(i))
		{
		    if(PlayerInfo[i][pLottoNr] > 0)
		    {
			    if(PlayerInfo[i][pLottoNr] == number)
			    {
					...
					format(string, sizeof(string), "Totolotek: %s Wygra≥ nagrodÍ w wysokoúci: $%d.", winner, Jackpot);
					OOCOff(COLOR_WHITE, string);
					PayLog(string);
					format(string, sizeof(string), "* Wygra≥eú nagrodÍ o wysokoúci: $%d dziÍki wytypowaniu prawid≥owej liczby !", Jackpot);
					SendClientMessage(i, COLOR_YELLOW, string);
			    	DajKase(i, Jackpot);
			    }
			    else
			    {
			        SendClientMessage(i, COLOR_LIGHTBLUE, "* Tym razem nie wygra≥eú w totolotku. SprÛbuj innym razem :) .");
			    }
			}
	...
	if(JackpotFallen)
	{
	    ...
	    Jackpot = rand;
	    SaveStuff();
	    format(string, sizeof(string), "Totolotek: Losowanie rozpoczÍte, przewidywana nagroda: $%d.", Jackpot);
		OOCOff(COLOR_WHITE, string);
		SendClientMessageToAll(COLOR_YELLOW, "Komora losowania jest pusta, proszÍ o zwolnienie blokady");
	}
	else
	{
	    ...
	    Jackpot += rand;
	    SaveStuff();
	    format(string, sizeof(string), "Totolotek: Nagroda zosta≥a podwyøszona do: $%d.", Jackpot);
		OOCOff(COLOR_WHITE, string);
	}
	return 1;
}
					...
					if(IsPlayerConnected(i))
					{
					    if(IsACop(i))
					    {
					        if(gCrime[i] == 0)
					        {
								...
								format(cbjstore, sizeof(turnmes), "HQ: PrzestÍpstwo: %s, Podejrzany: %s WL: %d",reason,turned, PoziomPoszukiwania[playerid]);
								SendClientMessage(i, COLOR_LFBI, cbjstore);
							}
						}
					}
...
/*SetPlayerWeapons(playerid)//megawaøne
{
	if(IsPlayerConnected(playerid))
	{
	    ResetPlayerWeapons(playerid);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "Level:[%d] P≥eÊ:[%s] Wiek:[%d] Pochodzenie:[%s] Zdrowie:[%.1f] Kasa:[$%d] Bank:[$%d] Telefon:[%d]", level,atext,age,otext,shealth+50, cash, account, pnumber);
		SendClientMessage(playerid, COLOR_GRAD1,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Konto Premium:[%s] ålub z:[%s] On-Line:[%d] LottoNr:[%d] Praca:[%s] Punkty karne:[%d]", drank,PlayerInfo[targetid][pMarriedTo],ptime,lotto,jtext, PlayerInfo[targetid][pPK]);
		SendClientMessage(playerid, COLOR_GRAD2,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Ryb Z≥owionych:[%d] NajwiÍksza Ryba:[%d] PrzestÍpstwa:[%d] Czas Aresztu:[%d] Smierci bÍdπc Poszukiwanym:[%d]", fishes,bigfish,crimes,arrests,warrests );
		SendClientMessage(playerid, COLOR_GRAD3,coordsstring);
		format(coordsstring, sizeof(coordsstring), "ZabiÊ:[%d] åmierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Rodzina:[%s] Skin ID:[%d]",kills,deaths,costlevel,exp,expamount,wanted,f2text, skin);
		SendClientMessage(playerid, COLOR_GRAD4,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Drugs:[%d] Mats:[%d] Frakcja:[%s] Ranga:[%s] Warny:[%d] DostÍpnych zmian nickÛw:[%d] Si≥a:[%d]",drugs,mats,ftext,rtext,PlayerInfo[targetid][pWarns],znick, PlayerInfo[targetid][pStrong]);
		...
		if(PlayerInfo[targetid][pPbiskey] >= 0 && PlayerInfo[targetid][pPbiskey] <= MAX_BIZNES)
		{
			...
			format(coordsstring, sizeof(coordsstring), "Si≥a:[%d] Biznes:[%s] MaxDochÛdBiz[%d]", BizData[bizid][eBizName], BizData[bizid][eBizMoney]);
			SendClientMessage(playerid, COLOR_GRAD5, coordsstring);
		}
...
stock orgInvitePlayer(playerid, orguid)
{
    ...
    gTeam[playerid] = 5;
    new lStr[64];
    format(lStr, 64, "ORG ª Zosta≥eú przyjÍty do organizacji %s.", OrgInfo[orgid][o_Name]);
...
stock orgUnInvitePlayer(playerid)
{
    ...
    new lStr[64];
    if(!orgIsValid(orgid)) return 0;
    format(lStr, 64, "ORG ª Zosta≥eú wyproszony z organizacji %s.", OrgInfo[orgid][o_Name]);
    SendClientMessage(playerid, COLOR_LIGHTBLUE, lStr);
    gPlayerOrgLeader[playerid] = false;
    return 1;
}
...
stock orgSetSpawnAtPlayerPos(playerid, orgid)
{
    ...
    SendClientMessage(playerid, COLOR_LIGHTBLUE, "ORG ª Spawn zmieniony.");
    return 1;
}
...
stock orgGivePlayerRank(playerid, callerid, rankid)
{
    ...
    if(!orgIsValid(orgid)) return 0;
    new lStr[128];
    format(lStr, 128, "ORG ª Otrzyma≥eú %d rangÍ (%s) w organizacji %s. Nada≥: %s.", rankid, (strlen(FamRang[PlayerInfo[playerid][pOrg]][rankid]) > 1) ? (FamRang[PlayerInfo[playerid][pOrg]][rankid]) : (FamRang[0][rankid]), OrgInfo[orgid][o_Name], (callerid == INVALID_PLAYER_ID) ? ("SYSTEM") : (GetNick(callerid)));
    SendClientMessage(playerid, COLOR_LIGHTBLUE, lStr);
...
NaprawSpojnoscWlascicielaDomu(playerid)
{
	...
	else if(domcheck == 0)
	{
		...
		{
			DajKase(playerid, (Dom[PlayerInfo[playerid][pDom]][hCena]/2));
			SendClientMessage(playerid, COLOR_PANICRED, "Wykro bug z dwoma w≥aúcicielami! Jesteú drugim w≥aúcicielem, wiÍc tracisz dom, otrzymujesz po≥owÍ wartosci domu.");
...
Niwelacje(playerid)
{
	...
	{
		format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d MATS”W DO 0$. JEØELI UWAØASZ, ØE NIES£USZNIE - ZG£Oå STRAT  NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pMats],PlayerInfo[playerid][pLevel]);
	...
	if(PlayerInfo[playerid][pLevel] == 1)
	{
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JEØELI UWAØASZ, ØE NIES£USZNIE - ZG£Oå STRAT  NA FORUM. AKUTALNY LVL: %d",kaska[playerid],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
			strins(kox, playername2, 0);
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JEØELI UWAØASZ, ØE NIES£USZNIE - ZG£Oå STRAT  NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pCash],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
		...
		{
			format(kox, sizeof(kox), " ZNIWELOWANO TWOJE %d$ DO 0$. JEØELI UWAØASZ, ØE NIES£USZNIE - ZG£Oå STRAT  NA FORUM. AKUTALNY LVL: %d",PlayerInfo[playerid][pAccount],PlayerInfo[playerid][pLevel]);
			SendClientMessage(playerid, COLOR_PANICRED, kox);
			strins(kox, playername2, 0);
...
stock GetVehicleMaxPassengers(iModel) //RydeR
{
    if(400 <= iModel <= 611)
    {
        static
...
stock ProceedSkinSelection(playerid, index, typ)
{
    ...
    if(ilosc <= 11)
    {
        ...
        xstart=640-floatmul(floatadd(w,margin),ilosc+calibrate)-(w*1.5); //Mnoøenie szerokosci i marginesu przez ilosc elementow w tablicy ze skinami
...
stock Zone_StartAttack(zoneid, attacker, defender)
{
        ...
        if(defender > 100) //gang-gang
        {
            foreach(Player, i)
            {
                if(GetPlayerOrg(i) == attacker-100)
                {
                    ...
                    SetPlayerCriminal(i, INVALID_PLAYER_ID, "Wojna gangÛw", false);
                }
    ...
    printf("[GangZone] Atak na strefÍ %d przez %d. Atakuje %d osÛb broni %d osÛb.", zoneid, attacker, ZoneAttackData[zoneid][0], ZoneAttackData[zoneid][1]);
}
...
stock Oil_Destroy(lID)
{
    ...
    foreach(Player, i)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[LSFD] Usunπ≥eú plamÍ oleju! Otrzymujesz 5 000$! [LSFD]");
        DajKase(i, 5000);
        SendFamilyMessage(17, COLOR_GREEN, "[LSFD] Straøak usunπ≥ plamÍ oleju! Na konto frakcji wp≥ywa 5 000$! [LSFD]");
        Sejf_Add(17, 5000);
    }
}
...
stock AddStrong(playerid, wartosc)
{
	...
	{
		sendTipMessage(playerid, "Error: Nie uda≥o siÍ zabraÊ wartoúci Si≥y - przekroczy 5k");
	}
...
stock TakeStrong(playerid, wartosc)
{
	...
	{
		sendTipMessage(playerid, "Error: Nie uda≥o siÍ zabraÊ wartoúci Si≥y");
	}
...
stock EndRunPlayer(playerid, wartosc)
{
	...
	sendTipMessage(playerid, "Gratulacje! UkoÒczy≥eú ca≥y bieg.");
	...
	new string[128];
	format(string, sizeof(string), "To twÛj %d bieg dziú", PlayerRunStat[playerid]);
	sendTipMessage(playerid, string);
	...
	if(PlayerInfo[playerid][pPbiskey] >= 0 && PlayerInfo[playerid][pPbiskey] <= MAX_BIZNES)
	{
		if(PlayerInfo[playerid][pDonateRank] > 2)
		{
			if(losuj == 1 || losuj == 2 || losuj == 3)//Max kwota
			{
				...
				SendClientMessage(playerid,COLOR_WHITE, string); 
				format(string, sizeof(string), "  DochÛd z biznesu: $%d", BizData[bizid][eBizMoney]);
				SendClientMessage(playerid,COLOR_WHITE, string);
				SendClientMessage(playerid, COLOR_WHITE, "Bonusy: {FFFF00}Konto Premium");
				SendClientMessage(playerid,COLOR_WHITE, "TwÛj dochÛd z biznesu {37AC45}osiπgnπ≥ maksimum"); 
			...
			if(losuj == 4 || losuj == 5 || losuj == 6 || losuj == 7)//Po≥owiczny zysk
			{
				...
				format(string, sizeof(string), "Nazwa biznesu: %s", BizData[bizid][eBizName]);
				SendClientMessage(playerid,COLOR_WHITE, string); 
				format(string, sizeof(string), "  DochÛd z biznesu: $%d", BizData[bizid][eBizMoney]/2);
				SendClientMessage(playerid,COLOR_WHITE, string);
				SendClientMessage(playerid, COLOR_WHITE, "Bonusy: {FFFF00}Konto Premium");
				SendClientMessage(playerid,COLOR_WHITE, "TwÛj dochÛd z biznesu {37AC45}osiπgnπ≥ po≥owÍ"); 
				SendClientMessage(playerid,COLOR_LIGHTBLUE, "|_________________________|");
			...
			if(losuj == 8 || losuj == 9)//minimalna strata
			{
				...
				SendClientMessage(playerid,COLOR_WHITE, string);
				SendClientMessage(playerid, COLOR_WHITE, "Bonusy: {FFFF00}Konto Premium");
				SendClientMessage(playerid,COLOR_WHITE, "TwÛj biznes przyniÛs≥ {FF0000}minimalne straty"); 
				SendClientMessage(playerid,COLOR_LIGHTBLUE, "|_________________________|");
				...
				else
				{
					...
					{
						sendTipMessageEx(playerid,COLOR_RED, "Twoje d≥ugi rosnπ! TwÛj biznes przynosi kolejne straty"); 
					}
				}
			
			}
			if(losuj == 10)//Duøa strata
			{
				...
				SendClientMessage(playerid,COLOR_WHITE, string);
				SendClientMessage(playerid, COLOR_WHITE, "Bonusy: {FFFF00}Konto Premium");
				SendClientMessage(playerid,COLOR_WHITE, "TwÛj biznes przyniÛs≥ {FF0000}duøe straty"); 
				SendClientMessage(playerid,COLOR_LIGHTBLUE, "|_________________________|");
		...
		else
		{
			...
			if(losuj == 9 || losuj == 10)//Duøa strata
			{
...
public OnPlayerTakeDamageWeaponHack(playerid, weaponid, fakekillid)
{
	...
	{
		new string[128];
		//MruDialog(playerid, "ACv2: Kod #2002", "Zosta≥eú wyrzucony za weapon hack.");
		//format(string, sizeof string, "ACv2 [#2002]: %s zosta≥ wyrzucony za weapon hack. | Moøliwy fakekill: %s .", GetNick(playerid, true), GetNick(fakekillid, true));
		//SendCommandLogMessage(string);
		format(string, sizeof string, "ACv2 [#2002]: %s moøe mieÊ weapon hack. | Jeúli fakekill, to: %s .", GetNick(playerid, true), GetNick(fakekillid, true));
		SendCommandLogMessage(string);
		//KickEx(playerid);
		return 1;
	}
...
stock StworzWjedz(playerid, Float:wjedzX, Float:wjedzY, Float:wjedzZ, Float:wyjedzX, Float:wyjedzY, Float:wyjedzZ, Float:RangePoint, VW, MessageIN[]=" ", MessageOut[]=" ", FracOwner=0, OrgOwner=0)
{
	...
	{	
		...
		{
			sendTipMessage(playerid, "Nie moøesz tutaj wjechaÊ!"); 
			return 1;
		}
	}
	...
	{
		sendTipMessageEx(playerid, COLOR_RED, "Nie jesteú w miejscu, w ktÛrym moøna to wpisaÊ"); 
	}
	return 1;
}*/
