...
public OznaczCzitera(playerid)
{
	...
	format(string, sizeof(string), "UWAGA! %s[%d] posiada s0beita na 99\%. Zbanuj go, gdy u�yje czit�w, b�d� po pewnym czasie gry, aby nie wy��czy� czit�w przy wej�ciu na serwer!", GetNick(playerid), playerid);
	SendAdminMessage(COLOR_PANICRED, string);
	SendAdminMessage(COLOR_PANICRED, "Gracz ten zosta� dodany do listy cziter�w, wpisz /cziterzy aby zobaczy� t� list�.");
	return 1;
}
...
stock Taxi_FareEnd(playerid)
{
	...
	format(string, sizeof(string), "* Zako�czy�e� s�u�b�, zarobi�e�: $%d.", TransportMoney[playerid]);
...
stock HABox(numer, playerid)
{
    ...
    {
        SendClientMessage(playerid, COLOR_LIGHTBLUE, "Kontrakt kt�ry podpisa�e� zosta� odebrany przez Hitman�w. Spodziewaj si� wykonania zlecenia.");
    }
	return 1;
}
...
public zabezpieczeniewoz(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
	    SendClientMessage(playerid, COLOR_PANICRED, "Dosta�e� kicka za prawdopodobie�stwo u�ycia czit�w. Je�li to bug, napisz na forum: www.mrucznik-rp.net");
		KickEx(playerid);
	}
	return 1;
}*/
...
KoniecWyscigu(playerid)
{
	...
	if(playerid == -1)
		...
		format(string, sizeof(string), "Komunikat wy�cigowy: {FFFFFF}Wyscig %s zako�czony przez %s.", Wyscig[Scigamy][wNazwa], GetNick(playerid));
...
public MRP_ShopPurchaseCar(playerid, model, cena)
{
    ...
    if(carid >= MAX_CARS)
    {
        SendClientMessage(playerid, COLOR_PANICRED, "Nie mo�na stworzy� pojazdu! Mo�liwe przepe�nienie, auto zosta�o kupione lecz nie mo�esz go u�y�.");
        return 1;
    }
