...
Do_WnetrzaWozu(playerid, vehicleid, model)
{
	...
	else if(model == 538)//Pociπg KT
	{
		if(PlayerInfo[playerid][pBiletsamolotowy] == 0)
		{
			sendErrorMessage(playerid, "Nie posiadasz biletu! Kup go na dworcu za pomocπ komendy /kbpo");
		}
...
Z_WnetrzaWozu(playerid, vehicleid)
{
	...
	{
		SetPlayerPosEx(playerid, vehx, vehy+0.23, vehz);
		sendTipMessage(playerid, "Marcepan Marks mÛwi: DziÍkujemy za podrÛøowanie naszymi liniami!");
	}
	else if(model == 582)//sanvan
	{
	    SetPlayerPosEx(playerid, vehx-1, vehy-1, vehz);
