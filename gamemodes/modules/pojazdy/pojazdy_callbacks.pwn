...
pojazdy_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == D_AUTO_ACTION)
	{
		...
		switch(listitem)
		{
			...
			{
				new string[2048];
				format(string, sizeof(string), "UsuÒ tuning - NITRO\nUsuÒ tuning - HYDRAULIKA\nUsuÒ tuning - FELGI\nUsuÒ tuning - MALUNEK\nUsuÒ tuning - SPOJLER\nUsuÒ tuning - ZDERZAKI");
				ShowPlayerDialogEx(playerid, D_AUTO_ACTION_TUNING, DIALOG_STYLE_LIST, "Panel pojazdu", string, "Wybierz", "Wyjdü");
			}
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 6:
				{
					...
					{
						MruMessageFail(playerid, "By kupiÊ w≥asnπ taksÛwkÍ, musisz mieÊ 4 skill kierowcy.");
						return 1;
					}
					ShowPlayerDialogEx(playerid, 4905, DIALOG_STYLE_MSGBOX, "Kupowanie Taxi", "Taxi\n\nCena: 1.500.000$\nPrÍdkoúÊ Maksymalna: 180km/h\nIloúÊ miejsc: 4\nOpis: Moøesz komercyjnie przewoziÊ ludzi! Wymaga pracy kierowcy i 4 skilla.", "Kup!", "WrÛÊ");
	...
	else if(dialogid == 400)//System ≥odzi - panel
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 8://Jacht
				{
					ShowPlayerDialogEx(playerid, 409, DIALOG_STYLE_MSGBOX, "Kupowanie Jachtu", "Jacht\n\nCena: 4.000.000$\nPrÍdkoúÊ Maksymalna: 80km/h\nWielkosc: Wielki\nOpis: Jacht to statek dla ludzi ktÛrzy wyprawiajπ siÍ w miÍdzykontynentalnπ przeprawÍ oraz pragnπ luksusu. Moøna w nim spaÊ, gdyø posiada spore wnÍtrze. Malowany w 2 kolorach.\n((UWAGA! Pojazd posiada wnÍtrze do ktÛrego moøna wchodziÊ komendπ /wejdzw))", "Kup!", "WrÛÊ");
