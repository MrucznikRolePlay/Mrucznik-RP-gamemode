...
//------------------<[ Implementacja: ]>-------------------
command_finisz_Impl(playerid, params[256])
{
    new string[128];
	...
	if(IsBusinessTypeOwnedByPlayerOrg(playerid, FRONT_BIZ_TYPE_RACE))
	{
		if(PlayerInfo[playerid][pRank] >= 4)
		{
		    if(tworzenietrasy[playerid] != 666)
		    {
		        ...
		        {
		            format(string, sizeof(string), "Postawiono metê. Wszystkich checkpointów: %d. Wyœcig pomyœlnie utworzony!", (Wyscig[tworzenietrasy[playerid]][wCheckpointy]+1));
		        ...
		        {
		            sendErrorMessage(playerid, "Wyœcig musi mieæ co najmniej 4 checkpointy!");
		        }
		    }
		    else
 			{
