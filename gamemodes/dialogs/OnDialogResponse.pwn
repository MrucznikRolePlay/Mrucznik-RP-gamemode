...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	else if(dialogid == 6996)
	{
		...
		{
			new string[128];
			switch(listitem)//Cytrynowy-sza≥\t10.000$\nW≥adca procÍtÛw\t15.000$\nNapÛj mistrza Cotty\t20.000$\nTwoja Stara\t 30.000$
			{
				case 0:
				{
					if(kaska[playerid] >= drinkCost1)
					{
						format(string, sizeof(string), "%s kupi≥ w barze %s - zaczyna piÊ", GetNick(playerid), drinkName1);
						...
						if(GetPVarInt(playerid, "jestPrzyBarzeVIP") == 1)
						{
							...
							Sejf_Save(FRAC_SN);
							sendTipMessageEx(playerid, COLOR_GREEN, "Jesteú VIP - p≥acisz o po≥owÍ mniej za swojego drinka!"); 
							return 1;
						}
				...
				case 1:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze %s - zaczyna powoli sπczyÊ!", GetNick(playerid), drinkName2);
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				...
				case 2:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze %s - zaczyna go piÊ. Cieknie mu po bluzce!", GetNick(playerid), drinkName3);
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				...
				case 3:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze %s bo chce byÊ rich", GetNick(playerid), drinkName4);
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
		...
		if(response)
		{
			...
			switch(listitem)
			{
				...
				//	SetActorAnimationEx(actorpID, "COP_AMBIENT", "Coplook_loop",  1, 1, 1);
					...
					sendTipMessage(playerid, string); 
					format(string, sizeof(string), "W≥πczy≥eú animacjÍ dla %s [%d]", Actors[actorpID][a_Name], actorpID);
					sendTipMessage(playerid, string); 
					format(string, sizeof(string), "Admin %s w≥πczy≥ animacjÍ dla Actora %s [%d]", GetNick(playerid), Actors[actorpID], actorpID); 
			...
			switch(listitem)
			{
				case 0:
				{
					for(new i; i<valActor; i++)
					{
						...
						{
							new string[124];
							format(string, sizeof(string), "Pomyúlnie zaznaczono actora %d - %s", i, Actors[i][a_Name]); 
							sendTipMessage(playerid, string);
				...
				case 1:
				{
					if(GetPVarInt(playerid, "pactorID") != 666)
					{
						...
						{
							sendErrorMessage(playerid, "ActorÛw nie moøna przenosiÊ pomiÍdzy interiorami!"); 
							return 1;
						}
						...
						sendTipMessage(playerid, "Pomyúlnie przeniesiono do Ciebie actora!"); 
					}
					else
					{
