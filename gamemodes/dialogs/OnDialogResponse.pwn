		    ...
		    if(response)
		    {
		        ...
		        switch(listitem)
		        {
		            case 0:
		            {
		                ...
		                ShowPlayerDialogEx(playerid, 496, DIALOG_STYLE_TABLIST, "Sejf organizacji - stan", stan, "WrÛÊ", "WrÛÊ");
		            }
		            case 1:
						ShowPlayerDialogEx(playerid, 497, DIALOG_STYLE_INPUT, "Sejf organizacji - wyp≥acanie", stan, "Wyp≥aÊ", "WrÛÊ");
		            case 2:
						ShowPlayerDialogEx(playerid, 498, DIALOG_STYLE_INPUT, "Sejf organizacji - wp≥acanie", stan, "Wp≥aÊ", "WrÛÊ");
					case 3:
						ShowPlayerDialogEx(playerid, 5430, DIALOG_STYLE_INPUT, "Sejf organizacji mats - wyp≥acanie", stanmats, "Wyp≥aÊ", "WrÛÊ");
					case 4: {
						format(stan, sizeof(stan), "%s\n{F8F8FF}IloúÊ materia≥Ûw ktÛre posiadasz:\t\t{008080}%d", stanmats, PlayerInfo[playerid][pMats]);
						ShowPlayerDialogEx(playerid, 5431, DIALOG_STYLE_INPUT, "Sejf organizacji mats - wp≥acanie", stan, "Wp≥aÊ", "WrÛÊ");
					}
		        }
		    }
		}
		...
		if(dialogid == 5431)
		{
		    if(response)
		    {
		        ...
		        else
		        {
					...
					ShowPlayerDialogEx(playerid, 5431, DIALOG_STYLE_INPUT, "Sejf organizacji mats - wp≥acanie", stanmats, "Wyp≥aÊ", "WrÛÊ");
		        }
		    }
		    else
		    {
