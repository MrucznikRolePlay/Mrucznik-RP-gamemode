		        ...
		        switch(listitem)
				{
				    case 0:
					{
						if (kaska[playerid] >= 60000000)
						{
							...
							format(string, sizeof(string), "%s[%d] cofnπ≥eú jednπ zmianÍ nicku. IloúÊ wykorzystanych zmian zobaczysz w /stats.",GetNick(playerid),PlayerInfo[playerid][UID]);
							SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
...
//----------------------------------
	...
	*/
	...
	if(dialogid==DIALOG_IBIZA_BILET)
	{
		...
		if(response)
		{
			//if(PlayerInfo[playerid][BankMoney] < IbizaBilet) return SendClientMessage(playerid, -1, "Nie masz wystarczajπcej iloúci pieniÍdzy");
