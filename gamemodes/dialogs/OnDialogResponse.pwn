		...
		if(dialogid == 495)
		{
		    if(response)
		    {
		        ...
		        switch(listitem)
		        {
					...
					case 4: {
						format(stan, sizeof(stan), "%s\nIloúÊ materia≥Ûw ktÛre posiadasz:\t\t{008080}%d", stanmats, PlayerInfo[playerid][pMats]);
		...
		if(dialogid == 5431)
		{
		    if(response)
		    {
		        ...
		        else
		        {
		            SendClientMessage(playerid, COLOR_P@, "Niepoprawna iloúÊ!");
					new stanmats[256];
					format(stanmats, sizeof(stanmats), "{F8F8FF}IloúÊ materia≥Ûw w sejfie:\t\t{008080}%d\nIloúÊ materia≥Ûw ktÛre posiadasz:\t\t{008080}%d", Rodzina_Mats[lider], PlayerInfo[playerid][pMats]);
