...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	else if(dialogid == 9522) //kara ban
	{
		...
		if(response && (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pZG] >= 4 || IsAScripter(playerid)))
		{
			...
			SendClientMessage(giveplayerid, COLOR_NEWS, "Jeúli uwaøasz ze ban jest nies≥uszny wejdü na www.Mrucznik-RP.pl i z≥Ûø prosbÍ o UN-BAN");
			Log(punishmentLog, INFO, "Admin %s ukara≥ %s karπ bana, powÛd: %s", 
			...
			{
				format(string, sizeof(string), "AdmCmd: Admin %s zbanowa≥ %s, powÛd: %s",  GetNickEx(playerid), giveplayerid, reason);
				SendPunishMessage(string, playerid); 
			}		
			//adminowe logi
			new str[128];
