	...
	else if(dialogid == 1099)
	{
		if(response)
		{
			...
			if(money > 0)
			{
				...
				//LOG
				format(bigstring, sizeof(bigstring), "Lider %s - %s [UID: %d] dokona≥ przelewu na konto %s w wysokoúci %d", 
				FractionNames[frakcja], 
				GetNick(playerid, true),
...
//=================[KONIEC]========================
	...
	else if(dialogid == 1010)
	{
		if(response)
		{
			...
			switch(listitem)
			{
				case 0:
				{
					...
					{
						sendErrorMessage(playerid, "Uøywasz juø tego slota"); 
						return 1;
					}
				}
				case 1:
		...
		if(!response)
		{
			sendTipMessage(playerid, "Wy≥πczy≥eú dialog ubioru"); 
			return 1;
		}
