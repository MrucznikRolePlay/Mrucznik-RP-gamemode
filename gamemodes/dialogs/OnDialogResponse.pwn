...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	else if(dialogid == DIALOG_UNFRAKCJA)
	{
		...
		if(!response)
		{
			...
			Log(adminLog, INFO, "Admin %s usunπ≥ gracza %s z frakcji %d - pozostawiajπc VLD.", GetPlayerLogName(playerid), GetPlayerLogName(id_Lidera), PlayerInfo[id_Lidera][pMember]);
		...
		if(dialogid == 696)
		{
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						if(!IsABOR(playerid)) return SendClientMessage(playerid, 0xB52E2BFF, "Te wejúcie jest tylko dla pracownikÛw.");
						SetPlayerPosEx(playerid, 1498.9341,-1537.0797,67.3069);
						SetPlayerVirtualWorld (playerid, 2);
						SendClientMessage(playerid, COLOR_LIGHTGREEN, "Poziom -1, Parking wewnÍtrzny");
					...
					case 1:
					{
						...
						TogglePlayerControllable(playerid,0);
						Wchodzenie(playerid);
						SendClientMessage(playerid, COLOR_LIGHTGREEN, "Poziom 0, Parking zewnÍtrzny");
						PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
					}
					case 2:
					{
		    ...
		    if(response)
		    {
		        ...
		        if(strval(inputtext) >= 1 && strval(inputtext) <= wkladanie)
		        {
		            if(strval(inputtext) <= kaska[playerid] )
		            {
			            ...
			            ShowPlayerDialogEx(playerid, 8002, DIALOG_STYLE_LIST, "Sejf - w≥Ûø", "GotÛwkÍ\nMateria≥y\nMarihuane\nHeroine", "Wybierz", "WrÛÊ");
			            ZapiszDom(PlayerInfo[playerid][pDom]);
						Log(payLog, INFO, "%s w≥oøy≥ do sejfu w domu %d kwotÍ %d$. W sejfie przed: %d$, po: %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), strval(inputtext), before, after);
					}
					else
					{
...
//By Simeone 25-12-2018
	...
	else if(dialogid == 1099)
	{
		if(response)
		{
			...
			if(money > 0)
			{
				...
				//LOG
				Log(payLog, INFO, "%s przela≥ do sejfu frakcji %d kwotÍ %d$. Nowy stan: %d$",
					GetPlayerLogName(playerid),
					frakcja,
					money,
	...
	else if(dialogid == 1076)//Wpisywanie kwoty i ca≥oúÊ funkcji - przelew z konta organizacji na gracza
	{
		...
		else
		{
			...
			Log(payLog, INFO, "%s przela≥ z sejfu frakcji %d na konto gracza %s kwotÍ %d$. Nowy stan: %d$",
				GetPlayerLogName(playerid),
				frakcja,
				GetPlayerLogName(giveplayerid),
				money,
...
//=================[KONIEC]========================
		...
		if(response)
		{
			...
			if(GetPVarInt(playerid, "bizWhatToDo") == 1)//Przyjmij pracownika
			{
				if(IsPlayerConnected(giveplayerid))
				{
					...
					{
						sendErrorMessage(playerid, "Ten gracz ma juø biznes!"); 
						return 1; 
					}
					if(playerid == giveplayerid)
					{
						sendErrorMessage(playerid, "Nie moøesz przyjπÊ samego siebie!"); 
						return 1;
					}
					PlayerInfo[giveplayerid ][pBusinessMember] = PlayerInfo[playerid][pBusinessOwner]; 
					format(string, sizeof(string), "Zosta≥eú przyjÍty do %s przez %s", Business[PlayerInfo[playerid][pBusinessOwner]][b_Name], GetNick(playerid));
					sendTipMessageEx(giveplayerid , COLOR_BLUE, string); 
					format(string, sizeof(string), "Przyjπ≥eú %s do swojego biznesu!", GetNick(giveplayerid)); 
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 2)//Zwolnij pracownika
			{
				if(IsPlayerConnected(giveplayerid))
				{
					...
					format(string, sizeof(string), "Zosta≥eú zwolniony z %s przez %s", Business[PlayerInfo[playerid][pBusinessOwner]][b_Name], GetNick(playerid));
					sendTipMessageEx(giveplayerid , COLOR_BLUE, string); 
					format(string, sizeof(string), "Zwolni≥eú %s ze swojego biznesu!", GetNick(giveplayerid )); 
					sendTipMessageEx(playerid, COLOR_BLUE, string); 
				}
			}
			else if(GetPVarInt(playerid, "bizWhatToDo") == 3)
			{
				...
				{
					sendErrorMessage(playerid, "Nie posiadasz przy sobie takiej gotÛwki!"); 
					return 1;
				}
				if(Business[PlayerInfo[playerid][pBusinessOwner]][b_moneyPocket]+valueMoney > 1_000_000_000)
				{
					sendErrorMessage(playerid, "Nie moøesz wsadziÊ takiej gotÛwki - sejf jest przepe≥niony!"); 
					return 1;
				}
				if(valueMoney <= 0)
				{
					sendErrorMessage(playerid, "WartoúÊ nie moøe byÊ podana z ''-''"); 
					return 1;
				}
				format(string, sizeof(string), "Wp≥aci≥eú do sejfu $%d, jest w nim teraz $%d", valueMoney, Business[PlayerInfo[playerid][pBusinessOwner]][b_moneyPocket]+valueMoney);
				sendTipMessageEx(playerid, COLOR_BLUE, string); 
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 4)
			{
				...
				{
					sendErrorMessage(playerid, "Nie posiadasz w sejfie biznesu takiej gotÛwki!"); 
					return 1;
				}
				if(valueMoney <= 0)
				{
	...
	else if(dialogid == D_BIZ_WRITE)
	{
		if(!response)
		{
			sendErrorMessage(playerid, "Wyszed≥eú z panelu tworzenia biznesu"); 
			return 1;
		}
		...
		if(response)
		{
			if(GetPVarInt(playerid, "MustBe") == 0)//Kwota kasy za jaka gracz ma kupic
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Mrucznik Role Play", "Wprowadü poniøej maksymalny zysk\nktÛry gracz bÍdzie mÛg≥ otrzymaÊ co godzine.", "Dalej", "Wyjdü"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 1)
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Mrucznik Role Play", "Wpisz poniøej nazwÍ biznesu", "Dalej", "Wyjdü"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 2)
			{
				if(strlen(inputtext) > 3 && strlen(inputtext) < 64)
				{
					...
					sendTipMessage(playerid, string); 
					sendTipMessage(playerid, "Teraz trzeba wybraÊ pozycjÍ wejúcia - staÒ w miejscu i wpisz /stworzbiznes"); 
					SetPVarInt(playerid, "ActionCreateBiz", 1); 
				}
				else
				{
					ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Mrucznik Role Play", "Wpisz poniøej nazwÍ biznesu\nMINIMALNIE 3 ZNAKI", "Dalej", "Wyjdü"); 
				}
				return 1;
			}
		}
