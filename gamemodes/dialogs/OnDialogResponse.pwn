	...
	if(dialogid == D_ERS_SPRZEDAZ_APTECZKI)
	{
		...
		if(response)
		{
			...
			else
			{
				...
				format(string, sizeof string, "%s kupi≥ od Ciebie apteczkÍ. Otrzymujesz %d$ prowizji.", PlayerName(playerid), prowizja);
				SendClientMessage(id, 0x0080D0FF, string);
				format(string, sizeof string, "Kupi≥eú apteczkÍ od Lekarza za %d$", HEALTH_PACK_PRICE);
				SendClientMessage(playerid, 0x00FF00FF, string);
				format(string, sizeof string, "[ERS] Lekarz sprzeda≥ apteczkÍ! Na konto frakcji wp≥ywa %d$", (HEALTH_PACK_PRICE - prowizja));
