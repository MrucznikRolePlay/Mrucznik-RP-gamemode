	...
	else if(dialogid == 6998)//vinyl input
	{
		...
		if(response)
		{
			if(GetPVarInt(playerid, "b-wybor") == 7)
			{
				...
				{
					sendErrorMessage(playerid, "Nieprawid�owa d�ugo�� nazwy napoju [MAX 33 znaki]");
				}
				return 1;
			}
			if(GetPVarInt(playerid, "b-wybor") == 6)
			{
				...
				ShowPlayerDialogEx(playerid, DIALOG_STYLE_INPUT, 6998, "Panel lidera", string, "Akceptuj", "Odrzu�"); 
			...
			if(GetPVarInt(playerid, "b-wybor") == 5)
			{
				...
				if(drinkID == 1) 
				{
					...
					format(string, sizeof(string), "Zmieni�e� cen� napoju %s. Nowa cena to: %d", drinkName1, drinkCost1); 
					sendTipMessage(playerid, string); 
				}
				else if(drinkID == 2)
				{
					drinkCost2 = drinkCost; 
					format(string, sizeof(string), "Zmieni�e� cen� napoju %s. Nowa cena to: %d", drinkName2, drinkCost2); 
					sendTipMessage(playerid, string);
				}
				else if(drinkID == 3)
				{
					drinkCost3 = drinkCost; 
					format(string, sizeof(string), "Zmieni�e� cen� napoju %s. Nowa cena to: %d", drinkName3, drinkCost3); 
					sendTipMessage(playerid, string);
				}
				else if(drinkID == 4)
				{
					drinkCost4 = drinkCost; 
					format(string, sizeof(string), "Zmieni�e� cen� napoju %s. Nowa cena to: %d", drinkName4, drinkCost4); 
					sendTipMessage(playerid, string);
				}
				else
				{
					sendErrorMessage(playerid, "Wyst�pi� nieznany problem. Skontaktuj si� z Komisj� ds. Ulepsze�"); 
				}
				return 1;
			}
		        ...
		        switch(listitem)
		        {
		            ...
		            case 2:
		            {
		                if(kaska[playerid] > 700)
		                {
		                	...
		                	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Kupi�e� kij baseballowy za 700$");
		                }
		                else
		                {
		                    SendClientMessage(playerid, COLOR_LIGHTBLUE, "Nie sta� ci�!");
		    ...
		    if(response)
		    {
		        ...
		        if(strval(inputtext) >= 1 && strval(inputtext) <= wkladanie)
		        {
		            if(strval(inputtext) <= PlayerInfo[playerid][pMats])
		            {
			            ...
			            ShowPlayerDialogEx(playerid, 8002, DIALOG_STYLE_LIST, "Sejf - w��", "Got�wk�\nMateria�y\nMarihuane\nHeroine", "Wybierz", "Wr��");
			            ZapiszDom(PlayerInfo[playerid][pDom]);
						Log(payLog, INFO, "%s w�o�y� do sejfu w domu %s paczk� %d materia��w", GetPlayerLogName(playerid), GetHouseLogName(dom), strval(inputtext));
					}
					else
					{
					    SendClientMessage(playerid, COLOR_P@, "Nie sta� ci� aby w�o�y� tyle do sejfu");
		    ...
		    if(response)
		    {
		        ...
		        if(strval(inputtext) >= 1 && strval(inputtext) <= wkladanie)
		        {
		            if(strval(inputtext) <= PlayerInfo[playerid][pDrugs])
		            {
			            ...
			            ShowPlayerDialogEx(playerid, 8002, DIALOG_STYLE_LIST, "Sejf - w��", "Got�wk�\nMateria�y\nMarihuane\nHeroine", "Wybierz", "Wr��");
			            ZapiszDom(PlayerInfo[playerid][pDom]);
						Log(payLog, INFO, "%s w�o�y� do sejfu w domu %s paczk� %d narkotyk�w ", GetPlayerLogName(playerid), GetHouseLogName(dom), strval(inputtext));
					}
					else
					{
					    SendClientMessage(playerid, COLOR_P@, "Nie sta� ci� aby w�o�y� tyle do sejfu");
		    ...
		    if(response)
		    {
		        ...
		        if((strlen(inputtext) >= 1 && strlen(inputtext) <= 9) && kasa > 0 )
		        {
		            if(kasa <= Sejf_Rodziny[lider])
		            {
			            ...
			            Log(payLog, INFO, "%s wyp�aci� z sejfu rodziny %d kwot� %d$. Nowy stan: %d$", 
							GetPlayerLogName(playerid),
							lider,
							kasa,
							Sejf_Rodziny[lider]);
		    ...
		    if(response)
		    {
		        ...
		        if((strlen(inputtext) >= 1 && strlen(inputtext) <= 9) && kasa > 0 )
		        {
		            if(kaska[playerid] >= kasa)
		            {
			            ...
			            SendClientMessage(playerid, COLOR_P@, komunikat);
			            Log(payLog, INFO, "%s wp�aci� do sejfu rodziny %d kwot� %d$. Nowy stan: %d$", 
							GetPlayerLogName(playerid),
							lider,
							kasa,
							Sejf_Rodziny[lider]);
...
//----------------------------------
	...
	*/
	...
	if(dialogid == D_MECH_SPRZEDAZ_FIXKIT)
	{
		...
		{
			SendClientMessage(id, 0xFF0030FF, sprintf("Gracz %s nie zgodzi� si� na kupno zestawu.", GetNick(playerid)));
		}
    ...
    else if(dialogid == D_EDIT_ORG_DELETE)
    {
        if(!response) return RunCommand(playerid, "/edytuj",  "");
        new id = GetPVarInt(playerid, "edit_org"), lStr[128];
        format(lStr, sizeof(lStr), "Usuni�to organizacj� %s.", OrgInfo[id][o_Name]);
        SendClientMessage(playerid, COLOR_GREEN, lStr);
        format(lStr, sizeof(lStr), "Organizacja usuni�ta przez %s.", GetNickEx(playerid));
        foreach(new i : Player)
        {
		...
		else
		{
			...
			if(money >= 1)
			{
				if(money <= kaska[playerid])
				{
					...
					Log(payLog, INFO, "%s wp�aci� na konto frakcji %d kwot� %d$. Nowy stan: %d$", 
						GetPlayerLogName(playerid),
						frakcja,
						money,
						Sejf_Frakcji[frakcja]);
		...
		else
		{
			...
			if(money >= 1)
			{
				if(money <= Sejf_Frakcji[frakcja])
				{
					...
					Log(payLog, INFO, "%s wyp�aci� z konta frakcji %d kwot� %d$. Nowy stan: %d$", 
						GetPlayerLogName(playerid),
						frakcja,
						money,
						Sejf_Frakcji[frakcja]);
...
//=================[KONIEC]========================
	...
	else if(dialogid == 1092)//Ca�uj - komenda - potwierdzenie
	{
		if(response)
		{
			...
			{
				new string[128];
				format(string, sizeof(string),"* %s kocha %s wi�c ca�uj� si�.", GetNick(playerid), GetNick(kissPlayerOffer[playerid]));
				ProxDetector(20.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
				format(string, sizeof(string), "%s m�wi: Kocham ci�.", GetNick(kissPlayerOffer[playerid]));
		...
		if(!response)
		{
			...
			new string[128];
			format(string, sizeof(string), "* %s spojrza�(a) na %s i stwierdzi�(a), �e nie chce si� ca�owa�!", GetNick(playerid), GetNick(kissPlayerOffer[playerid]));
			ProxDetector(20.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			...
			switch(listitem)
			{
				...
				case 3:
				{
					if(PlayerPersonalization[playerid][PERS_FAMINFO] == 0)
					{
						...
						sendTipMessage(playerid, "Wy��czy�e� komunikaty od rodzin!"); 
					}
					else
					{
						PlayerPersonalization[playerid][PERS_FAMINFO] = 0;
						sendTipMessage(playerid, "W��czy�e� komunikaty od rodzin!"); 
					}
				}
			}
		}
