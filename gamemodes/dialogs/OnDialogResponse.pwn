...
CheckDialogId(playerid, dialogid)
{
	...
	{
		new strError[124];
		format(strError, sizeof(strError), "Wystπpi≥ b≥πd z twoim dialogiem! - %d przypisany %d", dialogid, iddialog[playerid]);
		//Log(serverLog, WARNING, "B≥Ídne ID dialogu dla [gracz %d] dialogid - %d przypisany %d", playerid, dialogid,iddialog[playerid]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == D_VEHOPIS_UPDATE)
    {
        ...
        if(strlen(inputtext) < 4 || strlen(inputtext) > 120)
        {
		    ...
		    if ((inputtext[i] >= 'A' && inputtext[i] <= 'Z') || (inputtext[i] >= 'a' && inputtext[i] <= 'z') || (inputtext[i] >= '0' && inputtext[i] <= '9') || (inputtext[i] == ' ') || (inputtext[i] == ',') || (inputtext[i] == '.') || (inputtext[i] == '!') || (inputtext[i] == ':') || (inputtext[i] == '-') || (inputtext[i] == '{') || (inputtext[i] == '}') || (inputtext[i] == '[') || (inputtext[i] == ']'))
				continue;
            else if ((inputtext[i] == ' ') || (inputtext[i] == '”') || (inputtext[i] == '•') || (inputtext[i] == 'å') || (inputtext[i] == '£') || (inputtext[i] == 'Ø') || (inputtext[i] == 'è') || (inputtext[i] == '∆') || (inputtext[i] == '—') || (inputtext[i] == 'Í') || (inputtext[i] == 'Û') || (inputtext[i] == 'π') || (inputtext[i] == 'ú') || (inputtext[i] == '≥') || (inputtext[i] == 'ø') || (inputtext[i] == 'ü') || (inputtext[i] == 'Ê') || (inputtext[i] == 'Ò'))
                continue;
			else return SendClientMessage(playerid, COLOR_GRAD1, "Opis: Uøy≥eú nieodpowiednich znakÛw opisu.");
		}
        new veh = GetPlayerVehicleID(playerid);
    ...
    else if(dialogid == D_PANEL_KAR_BANIP)
    {
        ...
        format(str, 128, "ADM: %s - zablokowano IP: %s powÛd: %s", GetNick(playerid), inputtext, powod);
    ...
    else if(dialogid == D_PANEL_KAR_BANNICK)
    {
        ...
        format(str, 128, "ADM: %s - zablokowano nick: %s powÛd: %s", GetNick(playerid), inputtext, powod);
        SendClientMessage(playerid, COLOR_LIGHTRED, str);
		...
		if(response)
		{
			...
			switch(listitem)
			{
				...
				case 12:
				{
					...
					sendTipMessage(playerid, string); 
					format(string, sizeof(string), "Admin %s wyczyúci≥ animacje dla actora %s [%d]", GetNick(playerid), Actors[actorpID], actorpID); 
	...
	else if(dialogid == iddialog[playerid]) //TODO: WTF
	{
	    ...
	    if(dialogid == 122)
		{
			if(response == 1)
			{
			    switch(listitem)
			    {
			        ...
			        case 1:
			        {
				        SetPlayerPosEx(playerid,1450.6615,-1819.2279,77.9613);//g≥Ûwna sala urzÍdu
	    ...
	    else if(dialogid == 12)//shop 24-7
	    {
	        if(response)
	        {
				...
				if(mBiz[bIDE][b_elementsPocket] < BECOST_SHOP)
				{
					...
					sendErrorMessage(playerid, "Obecnie oczekujemy na dostawÍ towaru!"); 
					return 1;
				}
	            switch(listitem)
	            {
	                case 0://Zakup telefonu
					{
						...
						{
							SendBizLogoMessage(playerid, bIDE);
						    if(PlayerInfo[playerid][pTraderPerk] > 0)//Zniøka ze skilla
						    {
								...
								format(string,sizeof(string), "Zakupi≥eú Telefon za %d (cena przed zniøkπ %d)", payout, S_CENA_TELEFON);
						    ...
						    else
						    {
								...
								format(string,sizeof(string), "Zakupi≥eú Telefon za %d$", S_CENA_TELEFON);
								sendTipMessageEx(playerid, COLOR_WHITE, string);
							...
							format(string, sizeof(string), "TwÛj numer to: %d, moøesz go w kaødej chwili sprawdziÊ wpisujπc /stats", randphone);
							sendTipMessageEx(playerid, COLOR_GRAD4, string);
							sendTipMessageEx(playerid, COLOR_GRAD4, "WSKAZ”WKA: Wpisz /telefonpomoc aby zobaczyÊ komendy telefonu.");
					...
					case 2://Kupno ksiπøki telefonicznej
					{
						if (kaska[playerid] > S_CENA_KSIAZKA)
						{
							...
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessage(playerid, "Zakupi≥eú ksiπøkÍ telefonicznπ! Wpisz /numer <ID/Nick>"); 
							GetElementsFromBusiness(bIDE, BECOST_SHOP);
							return 1;
						}
					}
					case 3://Zakup kostki do gry
					{
						if (kaska[playerid] >= S_CENA_KOSTKA)
						{
							...
							gDice[playerid] = 1;
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessage(playerid, "Zakupi≥eú kostkÍ, moøesz niπ rzucaÊ za pomocπ /kostka2"); 
							GetElementsFromBusiness(bIDE, BECOST_SHOP);
							return 1;
						}
					}
					case 4://Kupno aparatu
					{
						if(kaska[playerid] > S_CENA_APARAT)
						{
							...
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessage(playerid, "Zakupi≥eú aparat z dyskiem na 100 zdjÍÊ! Moøesz robiÊ zdjÍcia.");
							GetElementsFromBusiness(bIDE, BECOST_SHOP);
							return 1;
						}
					}
					case 5://Kupno zamka
					{
						...
						{
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessageEx(playerid, COLOR_RED, "Brak towaru! Przyjdü pÛüniej.");
							return 1;
						}
					}
					case 6://Kupno prÍdkoúciomierza
					{
						if (kaska[playerid] > S_CENA_PREDKOSCIOMIERZ)
						{
							...
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessageEx(playerid, COLOR_RED, "Brak takiego towaru! Przyjdü pÛüniej.");
							return 1;
						}
					}
					case 7://Kupno kondoma
					{
						if (kaska[playerid] > S_CENA_KONDOM)
						{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Zakupi≥eú kondoma! Moøesz teraz uprawiaÊ bezpieczny sex (chyba)");
							GetElementsFromBusiness(bIDE, BECOST_SHOP);
							return 1;
						}
					}
					case 8://Kupno MP3
					{
						if (kaska[playerid] > S_CENA_ODTWARZACZ)
						{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Kupi≥eú odtwarzacz - aby skorzystaÊ z odtwarzacza MP3 wpisz /mp3 bπdü /muzyka"); 
					...
					case 9://Kupno piwa
					{
						if (kaska[playerid] > S_CENA_PIWO)
						{
						    ...
						    {
						    	sendErrorMessage(playerid, "Nie masz tyle si≥y aby udüwignπÊ kolejne piwo!"); 
								return 1;
						    }
						    if(PlayerInfo[playerid][pTraderPerk] > 0)
					    	{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Piwo mruczny Gul zakupione! Aby wypiÊ wpisz /piwo"); 
							GameTextForPlayer(playerid, string, 5000, 1);
					...
					case 10://Kupno wina
					{
						if (kaska[playerid] > S_CENA_WINO)
						{
						    ...
						    {
						    	sendErrorMessage(playerid, "Nie masz na tyle si≥y, aby udüwignπÊ kolejne wiÒsko!"); 
								return 1;
						    }
						    if(PlayerInfo[playerid][pTraderPerk] > 0)
					    	{
							...
							GameTextForPlayer(playerid, string, 5000, 1);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Wino Komandos zosta≥o kupione, aby uøyÊ wpisz /wino!"); 
					...
					case 11://Kupno sprunka
					{
						if (kaska[playerid] >= S_CENA_SPRUNK)
						{
						    ...
						    {
						    	sendErrorMessage(playerid, "Masz zbyt ma≥o si≥y, aby udüwignπÊ kolejnego sprunka!");
								return 1;
						    }
						    if(PlayerInfo[playerid][pTraderPerk] > 0)
					    	{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Zakupi≥eú sprunka. Aby wypiÊ wpisz /sprunk"); 
					...
					case 12://CB_RADIO
					{
						if (kaska[playerid] > S_CENA_CBRADIO)
						{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Zakupi≥eú CB-RADIO. Aby uøyÊ wpisz w pojeüdzie /cb [text]"); 
							sendTipMessage(playerid, "Aby wyciszyÊ CB-RADIO wpisz /togcb, aby wyrzuciÊ /wywal"); 
					...
					case 13://Kupno cygar
					{
						if (kaska[playerid] > S_CENA_CYGARO)
						{
						    ...
						    {
						    	sendErrorMessage(playerid, "Posiadasz ca≥π paczkÍ cygar!");
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE); 
							sendTipMessage(playerid, "Zakupi≥eú paczkÍ 5-u cygar. Aby zapaliÊ wpisz /cygaro"); 
						...
						{
							sendErrorMessage(playerid, "Nie posiadasz takiej iloúci gotÛwki!"); 
						}
					}
					case 14://Kupno kija
					{
						if (kaska[playerid] >= S_CENA_KIJ )
						{
							...
							PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
							SendBizLogoMessage(playerid, bIDE);
							sendTipMessage(playerid, "Zakupi≥eú Kij Baseballowy, moøesz teraz t≥uc innych!"); 
							GetElementsFromBusiness(bIDE, BECOST_SHOP);
							return 1;
						}
						else
						{
							sendErrorMessage(playerid, "Nie posiadasz wystarczajπcej iloúci gotÛwki!"); 
						}
					}
					case 15://kupno nawigacji
					{
						...
						GetElementsFromBusiness(bIDE, BECOST_SHOP);
						sendTipMessage(playerid, "Zakupi≥eú nawigacjÍ do samochodu! Aby uøyÊ wpisz /mnawigacja (skrÛt: /mnaw)"); 
					}
				}
			}
		}
		        ...
		        switch(listitem)
		        {
		            ...
		            case 2:
		            {
		                if(kaska[playerid] > 700)
		                {
		                	...
		                	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Kupi≥eú bejzbola za 700$");
		                }
		                else
		                {
		                    SendClientMessage(playerid, COLOR_LIGHTBLUE, "Nie staÊ ciÍ!");
		...
		if(dialogid == 85)//system domÛw
		{
		    if(response)
		    {
	     		ShowPlayerDialogEx(playerid, 86, DIALOG_STYLE_MSGBOX, "Kupowanie domu - p≥atnoúÊ", "P≥atnoúci chcesz dokonaÊ gotÛwkπ czy przelewem z banku?", "GotÛwka", "Przelew");
		    }
		}
		    ...
		    if(response)
		    {
	   			...
	   			if(/*PlayerInfo[playerid][pZiolo] >= 10 &&*/ PlayerInfo[playerid][pDrugs] >= 10 && PlayerInfo[playerid][pPiwo] >= 1 && HP >= 100 && STDPlayer[playerid] == 0)
			    {
	      			if(kaska[playerid] != 0 && kaska[playerid] >= 100000)
		        	{
						...
						//Dom[dom][hMagazyn] --;
						SendClientMessage(playerid, COLOR_P@, "Kupi≥eú ApteczkÍ za 100 000$, piwo oraz 10g marihuany i heroiny. Aby jej uøyÊ wpisz /ulecz");
						KupowanieDodatkow(playerid, dom);
		    ...
		    if(response)
		    {
		        ...
		        switch(listitem)
		        {
		            case 0:
		            {
		                ...
		                ShowPlayerDialogEx(playerid, 496, DIALOG_STYLE_LIST, "Sejf rodzinny - stan", stan, "WrÛÊ", "WrÛÊ");
		            }
		            case 1:
		            {
		                new stan[128];
		...
		if(dialogid == 498)
		{
		    if(response)
		    {
		        ...
		        if((strlen(inputtext) >= 1 && strlen(inputtext) <= 9) && kasa > 0 )
		        {
					...
					{
					    SendClientMessage(playerid, COLOR_P@, "Nie masz aø tyle przy sobie !");
					    new stan[256];
		                format(stan, sizeof(stan), "{F8F8FF}Stan sejfu:\t{008000}%d$", Sejf_Rodziny[lider]);
						ShowPlayerDialogEx(playerid, 498, DIALOG_STYLE_INPUT, "Sejf rodzinny - wp≥acanie", stan, "Wp≥aÊ", "WrÛÊ");
					}
        ...
        else if(dialogid == 1401)
		{
		    if(response)
		    {
		        ...
		        switch(listitem)
		        {
		            case 0://Bia≥y
		            {
						format(string, sizeof(string), "Bia≥e neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
		            ...
		            case 1://ØÛ≥ty
		            {
						format(string, sizeof(string), "ØÛ≥te neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
		            ...
		            case 2://Zielony
		            {
						format(string, sizeof(string), "Zielone neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
		            ...
		            case 3://Niebieski
		            {
						...
						format(string, sizeof(string), "Niebieskie neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
						SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
		            ...
		            case 4://Czerwony
		            {
						...
						format(string, sizeof(string), "Czerwone neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
		            ...
		            case 5://RÛøowy
		            {
						format(string, sizeof(string), "RÛøowe neony zosta≥y zamontowane w twoim %s. Koszt: 3 500 000$. Aby je w≥πczyÊ wpisz /dr", VehicleNames[GetVehicleModel(GetPlayerVehicleID(playerid))-400]);
		    ...
		    if(response)
		    {
		        ...
		        switch(listitem)
		        {
		            ...
		            case 0:
		            {
						...
						{
							format(string, sizeof(string), "Pani Janina mÛwi: Oto pakiet 50 kredytÛw za jedyne %d$.", onePoolPrice);
							SendClientMessage(playerid, COLOR_WHITE, string);
							Kredyty[playerid] += 50;
		            ...
		            case 1:
		            {
						...
						{
							format(string, sizeof(string), "Pani Janina mÛwi: Oto pakiet 100 kredytÛw za jedyne %d$.", twoPoolPrice);
							SendClientMessage(playerid, COLOR_WHITE, string);
		            ...
		            case 2://Zielony
		            {
						...
						{
							format(string, sizeof(string), "Pani Janina mÛwi: Oto pakiet 250 kredytÛw za jedyne %d$.", threePoolPrice);
							SendClientMessage(playerid, COLOR_WHITE, string);
		            ...
		            case 3://Niebieski
		            {
						...
						{
							format(string, sizeof(string), "Pani Janina mÛwi: Oto pakiet 500 kredytÛw za jedyne %d$.", fourPoolPrice);
							SendClientMessage(playerid, COLOR_WHITE, string);
				...
				switch(listitem)
				{
					case 0://OtwÛrz/zamknij basen
					{
						...
						else
						{
							...
							format(string, sizeof(string), "%s zamknπ≥ basen - Koniec p≥ywania!", GetNick(playerid, true));
							SendNewFamilyMessage(43, TEAM_BLUE_COLOR, string);
					...
					{
						format(string, sizeof(string), "%s uøy≥ komunikatu basenu", GetNick(playerid, true));
		...
		if(dialogid == 1417)//usuÒ trase
		{
		    if(response)
			{
				Log(commandLog, INFO, "%s zlikwidowa≥ trasÍ wyúcigowπ %s[%d]", GetPlayerLogName(playerid), Wyscig[tworzenietrasy[playerid]][wNazwa], tworzenietrasy[playerid]);
...
//----------------------------------
    ...
    else if(dialogid == D_EDIT_ORG_DELETE)
    {
        ...
        SendClientMessage(playerid, COLOR_GREEN, lStr);
        format(lStr, sizeof(lStr), "Organizacja usuniÍta przez %s.", GetNick(playerid));
        foreach(new i : Player)
        {
			...
			case 2:
			{
				for(new i; i<=MAX_BIZ; i++)
				{
					...
					if(checkedBiz == loadedBiz)
					{
						...
						{
							sendTipMessage(playerid, "Nie znaleziono biznesu, ktÛry potrzebuje dostawy!"); 
							return 1;
						}
						...
						wynagrodzenie); 
						ShowPlayerDialogEx(playerid, DIALOG_AKCEPT_ZLECENIE, DIALOG_STYLE_MSGBOX, SetDefaultCaption(), lStr, "Akceptuj", "OdmÛw"); 
		...
		if(response)
		{
			...
			if(money > 0)
			{
				...
				if(money >= 2_500_000)
				{
					...
					format(string, sizeof(string), "Przelewajπcy: %s [%d]", GetNick(playerid, true), playerid);
...
//=================[KONIEC]========================
	else if(dialogid == 1090)//Dialog do kupna biletÛw KT --> Pociπg
	{
        ...
        {
			new string[128];
            format(string, sizeof(string), "* %s jest strasznie rozstargniony i zdecydowa≥ odejúÊ od maszyny bez biletu.", GetNick(playerid, true));//Ciekawostka - niezdecydowany
			ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
			return 1;
        }
		else
		{
			if(IsAtTicketMachine(playerid))
			{
				...
				if(kaska[playerid] >= CenaBiletuPociag)				        
				{
					...
					format(string, sizeof(string), "* %s zakupi≥ bilet do pociπgu za %d$, schowa≥ go do kieszeni.", GetNick(playerid, true), CenaBiletuPociag);
					ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				}
				else
				{
	...
	else if(dialogid == 1092)//Ca≥uj - komenda - potwierdzenie
	{
		if(response)
		{
			...
			{
				new string[128];
				format(string, sizeof(string),"* %s kocha %s wiÍc ca≥ujπ siÍ.", GetNick(playerid, true), GetNick(kissPlayerOffer[playerid], true));
				ProxDetector(20.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
				format(string, sizeof(string), "%s mÛwi: Kocham ciÍ.", GetNick(kissPlayerOffer[playerid], true));
				ProxDetector(20.0, playerid, string, COLOR_WHITE,COLOR_WHITE,COLOR_WHITE,COLOR_WHITE,COLOR_WHITE);
				format(string, sizeof(string), "%s mÛwi: Ja ciebie teø.", GetNick(playerid, true));
				ProxDetector(20.0, playerid, string, COLOR_WHITE,COLOR_WHITE,COLOR_WHITE,COLOR_WHITE,COLOR_WHITE);
		...
		if(!response)
		{
			...
			new string[128];
			format(string, sizeof(string), "* %s spojrza≥(a) na %s i stwierdzi≥(a), øe nie chce siÍ ca≥owaÊ!", GetNick(playerid, true), GetNick(kissPlayerOffer[playerid], true));
			ProxDetector(20.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
