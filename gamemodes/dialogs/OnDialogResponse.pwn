	...
	else if(dialogid == D_PERSONALIZE)
	{
		...
		else if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					sendTipMessageEx(playerid, COLOR_P@, "Ta opcja zostanie w≥πczona juø nied≥ugo!"); 
					if(PlayerPersonalization[playerid][PERS_KB] == 0)
					{
						sendTipMessage(playerid, "Wy≥πczy≥eú konto bankowe"); 
						PlayerPersonalization[playerid][PERS_KB] =1;
					}
					else if(PlayerPersonalization[playerid][PERS_KB] == 1)
					{
						sendTipMessage(playerid, "W≥πczy≥eú konto bankowe"); 
