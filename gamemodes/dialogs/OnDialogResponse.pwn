...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid != iddialog[playerid])
    {
        ...
        #if defined DEBUG
        printf("B≥Ídne ID dialogu dla [%d] aktualny [%d] przypisany %d", playerid, dialogid,iddialog[playerid]);
        #endif
        return 1;
    }
		...
		if(response)
		{
			...
			switch(listitem)//Cytrynowy-sza≥\t10.000$\nW≥adca procÍtÛw\t15.000$\nNapÛj mistrza Cotty\t20.000$\nTwoja Stara\t 30.000$
			{
				case 0:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze Cytrynowy Sza≥ i zaczyna go piÊ", GetNick(playerid));
				...
				case 1:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze W≥adce procentÛw i zaczyna go piÊ", GetNick(playerid));
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				...
				case 2:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze NapÛj Cotty (chce byÊ jak L.Cotta) i zaczyna go piÊ", GetNick(playerid));
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				...
				case 3:
				{
					...
					{
						format(string, sizeof(string), "%s kupi≥ w barze twoja stara i zaczyna go piÊ jak kozak", GetNick(playerid));
						ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
...
//----------------------------------
	...
	if(dialogid==DIALOG_IBIZA_WPLATA)
	{
		if(response)
		{
			new hajs = strval(inputtext);
			if(hajs<=0) return SendClientMessage(playerid, 0xFF0000FF, "Niepoprawna kwota");
			if(hajs > GetPlayerMoney(playerid)) return SendClientMessage(playerid, -1, "Nie masz tyle hajsu"); //HAJS zamieniÊ GetPlayerMoney na pobranie ze struktury
        ...
        if(response)
        {
			switch(listitem)
			{
			    case 0:
				{
					...
					else sendErrorMessage(playerid, "Ten poziom zosta≥ zablokowany przez pracownika Sπdu!");
				}
				case 1:
    			{
					...
					if(lUID != 0)
					{
						foreach(new i : Player)
						{
							if(PlayerInfo[i][pUID] == lUID)
							{
								...
								SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
								format(string, sizeof(string), "%s usunπ≥ pojazd %s ze slotu %d (UID: %d) /edytuj - pojazd", GetNick(playerid), GetNick(i), lSlot, lUID);
